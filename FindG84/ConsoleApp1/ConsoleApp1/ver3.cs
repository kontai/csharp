...
namespace ConsoleApp1
{
    internal class Program
    {
        ...
        static void ProcessDirectoryParallel(DirectoryInfo rootDir, StreamWriter outputWriter)
        {
            ...
            // 保留一些 CPU 核心給系統使用,避免系統卡頓
            int maxThreads = Math.Max(1, Environment.ProcessorCount - 1); // 保留 1 個核心
            Console.WriteLine($"使用 {maxThreads} 個執行緒進行並行處理 (共 {Environment.ProcessorCount} 個核心)");
