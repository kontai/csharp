...
//dx=0,dy=0,size={2},x_anchor=0,y_anchor=0,rotation=0,mirror=none";
        ...
        )
        {
            ...
            List<string> signalLayers = new List<string>();
            Gen.INFO($"-t step -e {job}/{step} -m script -d LAYERS_LIST");
            ...
            foreach (var lyr in Gen.GetInfo("gLAYERS_LIST"))
            {
                ...
                int signalCount = signalLayers.Count; // Count of signal layers
                if (signalCount > 0)
                {
                    outer.Add(signalLayers[0]);
                    if (signalCount > 1)
                    {
                        outer.Add(signalLayers[signalCount - 1]);
                        if (signalCount > 2)
                        {
                            for (int i = 1; i < signalCount - 1; i++)
                            {
                                inner.Add(signalLayers[i]);
                            }
                        }
                    }
                }
