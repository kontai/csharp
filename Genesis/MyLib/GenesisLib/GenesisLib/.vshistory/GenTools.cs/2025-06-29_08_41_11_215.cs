...
namespace GenesisLib
{
    ...
    {
        public readonly static Dictionary<string,string>Type= new Dictionary<string, string>()
        {
            {"sig","single"},
            {"sm","solder_mask"},
            {"ss","silk_screen"},
...
//dx=0,dy=0,size={2},x_anchor=0,y_anchor=0,rotation=0,mirror=none";
        ...
        public static void Layer_Create(string layer,string type)=>
            Gen.COM($"create_layer,layer={layer},context=board,type={type},polarity=positive,ins_layer=,location=before");
        ...
        public static LayerCollection FindAllLayers(string job, string step)
        {
            ...
            int signalCount = layers.SignalTMP.Count;
            if (signalCount > 0)
            {
                try
                {
                    ...
                    if (signalCount > 1)
                    {
                        ...
                        if (signalCount > 2)
                        {
                            layers.Inner.AddRange(layers.SignalTMP.GetRange(1, signalCount - 2));
                        }
                    }
                }
                catch (Exception ex)
        ...
        public static void MyFindAllLayers(string job,
            string step,
