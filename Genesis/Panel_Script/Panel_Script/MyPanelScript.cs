using GenesisLib;
using System;
using System.Collections.Generic;
using System.IO;
using System.Text.RegularExpressions;
using System.Windows;

//TODO: 固定板框min_x, min_y, max_x, max_y ,將裁板尺寸納入計算
namespace Panel_Script
{
    public static class MyPanelScript
    {
        /// <summary>
        /// OUT_INNER 枚舉類型，用於表示外層或內層
        /// <para>
        /// OUTER: 外層
        /// </para>
        /// <para>
        /// INNER: 內層
        /// </para>
        /// </summary>
        private enum ENUM_PIN_LYR
        {
            OUTTER,
            INNER,
            SMSSTENTVIA,
            DRLINNGBX,
        }

        private static MySymbols sym;
        private static MainWindow mainWindow;
        private static readonly LayerCollection layers = MainWindow.layers;
        private static readonly int LayerCount = layers.SignalLayers.Count;
        private static readonly string workName = "work";
        private static string job = Gen.JOB;
        private static string step = Gen.STEP;
        private static readonly double s_ix; // 用於存儲成型尺寸 X 坐標值
        private static double s_iy; // 用於存儲成型尺寸 Y 坐標值
        private static double w_ix; // 用於存儲下料尺寸 X 坐標值
        private static double w_iy; // 用於存儲下料尺寸 Y 坐標值
        private static int xstep_num;
        private static int ystep_num;
        private static double xstep_spec;
        private static double ystep_spec;
        private static double frame_x; //排板尺寸X
        private static double frame_y; //排板尺寸Y
        private static double frame_dist_x; //下料邊X
        private static double frame_dist_y; //下料邊Y
        private static double min_x; //主板尺寸X (virtual size for static panel)
        private static double min_y; //主板尺寸Y (virtual size for static panel)
        private static double max_x; //最大尺寸X (virtual size for static panel)
        private static double max_y; //最大尺寸Y (virtual size for static panel)
        private static readonly double static_frame_width_x = 0.8; //固定板框虛擬寬度X
        private static readonly double static_frame_width_y = 0.8; //固定板框虛擬寬度Y
        private static double centerX;
        private static double centerY;
        public static string Job => job;

        public static string Step => step;

        //gererate all fields property
        public static double Six => s_ix; //排板尺寸X

        public static double Siy => s_iy; //排板尺寸Y

        public static double Wix => w_ix; //下料尺寸X

        public static double Wiy => w_iy; //下料尺寸Y
        public static double CenterX => centerX;
        public static double CenterY => centerY;

        public static int Xstep_num => xstep_num; // 排板數量x
        public static int Ystep_num => ystep_num; // 排板數量y

        public static double Xstep_spec => xstep_spec; // 排板間距X

        public static double Ystep_spec => ystep_spec; // 排板間距Y

        public static double Frame_x => frame_x; //排板尺寸X(包括邊距)
        public static double Frame_y => frame_y; //排板尺寸Y(包括邊距)
        public static double Frame_Dist_x => frame_dist_x; //下料邊X
        public static double Frame_Dist_y => frame_dist_y; //下料邊Y
        public static double Min_x => min_x; //主板尺寸X (virtual size for static panel)
        public static double Min_y => min_y; //主板尺寸Y (virtual size for static panel)
        public static double Max_x => max_x; //最大尺寸X (virtual size for static panel)
        public static double Max_y => max_y; //最大尺寸Y (virtual size for static panel)

        static MyPanelScript()
        {
            //初始化
            layers = MainWindow.layers;
            mainWindow = Application.Current.MainWindow as MainWindow;
            double mm2Inch = mainWindow.unit_mminch.Text == "MM" ? 0.03937 : 1.0; //毫米轉英寸
            s_ix = GenTools.DoubleRound(double.Parse(mainWindow.six.Text) * mm2Inch);
            s_iy = GenTools.DoubleRound(double.Parse(mainWindow.siy.Text) * mm2Inch);
            w_ix = GenTools.DoubleRound(double.Parse(mainWindow.wix.Text) * mm2Inch);
            w_iy = GenTools.DoubleRound(double.Parse(mainWindow.wiy.Text) * mm2Inch);
            xstep_num = int.Parse(mainWindow.xstep_num.Text);
            ystep_num = int.Parse(mainWindow.ystep_num.Text);
            xstep_spec = GenTools.DoubleRound(double.Parse(mainWindow.xstep_spec.Text) * mm2Inch);
            ystep_spec = GenTools.DoubleRound(double.Parse(mainWindow.ystep_spec.Text) * mm2Inch);
        }

        //雙精度的四捨五入方法

        //GenTools. DoubleRound 方法將數字四捨五入到小數點後兩位

        public static void RunPanelScript()
        {
            //hide mainwindow
            //mainWindow.Hide();
            mainWindow.Focusable = false;

            string selStep = mainWindow.stpSlect.Text;

            #region open work step and check size

            //手動排板
            if (selStep == workName)
            {
                //開啟work step
                if (Step != workName)
                {
                    Gen.COM($"open_entity,job={Job},type=step,name={workName},iconic=no");
                }

                frame_x = Six;
                frame_y = Siy;

                // If the step already exists, open it
                frame_dist_x = GenTools.DoubleRound((Wix - s_ix) / 2.0);
                frame_dist_y = GenTools.DoubleRound((Wiy - s_iy) / 2.0);

                //判斷下料尺寸是否大於成型尺寸
                if (frame_dist_x < 0 || frame_dist_y < 0)
                {
                    Gen.PAUSE("Size not correct,please check");
                    // Exit the script if the size is not correct
                    return;
                }

                //清空所有圖層
                Gen.COM("affected_layer,mode=all,affected=yes");
                Gen.COM("sel_delete");
                Gen.COM("affected_layer,mode=all,affected=no");
            }
            else if (selStep != workName)
            {
                frame_x = GenTools.DoubleRound((Six * xstep_num) + ((xstep_num - 1) * xstep_spec));
                frame_dist_x = GenTools.DoubleRound((Wix - frame_x) / 2.0);
                frame_y = GenTools.DoubleRound((Siy * ystep_num) + ((ystep_num - 1) * ystep_spec));
                frame_dist_y = GenTools.DoubleRound((Wiy - frame_y) / 2.0);
                if (frame_dist_x < 0 || frame_dist_y < 0)
                {
                    Gen.PAUSE("Size not correct,please check");
                    // Exit the script if the size is not correct
                    return;
                }

                Gen.INFO($" -t job -e {Job} -m script -d STEPS_LIST");
                List<string> MyStepsList = new List<string>(Gen.GetInfo("gSTEPS_LIST"));
                if (MyStepsList.Contains(workName))
                {
                    //Gen.PAUSE($"{workName} is exist, delete id");
                    MessageBoxResult result = MessageBox.Show(
                        $"{workName} 已存在,是否刪除？",
                        "確認刪除",
                        MessageBoxButton.YesNo,
                        MessageBoxImage.Question);
                    if (result == MessageBoxResult.Yes)
                    {
                        Gen.COM($"delete_entity,job={Job},type=step,name={workName}");
                    }
                    else
                    {
                        return;
                    }
                }

                Gen.COM($"create_entity,job={Job},is_fw=no,type=step,fw_type=form,name={workName},db=");
                Gen.COM($"open_entity,job={Job},type=step,name={workName},iconic=no");

                AutoPanel();
            }

            if (mainWindow.StaticFrame.IsChecked == true)
            {
                if (Frame_Dist_x > static_frame_width_x)
                {
                    min_x = -frame_dist_x + static_frame_width_x;
                    max_x = Frame_x + Frame_Dist_x - static_frame_width_x;
                }
                else
                {
                    min_x = 0;
                    max_x = Frame_x;
                }

                if (Frame_Dist_y > static_frame_width_y)
                {
                    min_y = -frame_dist_y + static_frame_width_y;
                    max_y = Frame_y + Frame_Dist_y - static_frame_width_y;
                }
                else
                {
                    min_y = 0;
                    max_y = Frame_y;
                }
            }
            else
            {
                min_x = 0;
                min_y = 0;
                max_x = Frame_x;
                max_y = Frame_y;
            }

            //板中心
            centerX = GenTools.DoubleRound(Frame_x / 2.0);
            centerY = GenTools.DoubleRound(Frame_y / 2.0);
            step = workName;

            #endregion open work step and check size

            //單位設定
            Gen.COM("units,type=inch");

            #region clear layer , create layer pin inn via tent

            //清空內容
            // clear all layer befor run script
            GenTools.Clear_Layers();

            //判斷一些必要層別
            //if (!GenTools.Layer_IsExist(Job, Step, "gbx"))
            //{
            //    Gen.PAUSE("gbx Layer not exist. please check");
            //    return;
            //}

            //if (!GenTools.Layer_IsExist(Job, Step, "drl"))
            //{
            //    Gen.PAUSE("drl Layer not exist. please check");
            //    return;
            //}

            //多層板,建立inn
            if (LayerCount > 2)
            {
                if (!GenTools.Layer_IsExist(Job, workName, "inn"))
                    GenTools.Layer_Create("inn", "board", GenTools.layerType["drill"]);
                if (!GenTools.Layer_IsExist(Job, workName, "pin"))
                    GenTools.Layer_Create("pin", "board", GenTools.layerType["drill"]);
            }

            //多層板,建立 t1,v1
            if (layers.Via.Count == 0)
            {
                GenTools.Layer_Create("v1", "misc", GenTools.layerType["doc"]);
                layers.Via.Add("v1");
            }

            if (layers.Tent.Count == 0)
            {
                GenTools.Layer_Create("t1", "misc", GenTools.layerType["doc"]);
                layers.Tent.Add("t1");
            }

            #endregion clear layer , create layer pin inn via tent

            //create the frame
            Gen.COM($"profile_rect" +
                    $",x1={-Frame_Dist_x},y1={-Frame_Dist_y}" +
                    $",x2={Frame_x + Frame_Dist_x},y2={Frame_y + Frame_Dist_y}");

            //內、外框鋪銅
            Do_Patttern_Fill();

            //印刷PIN
            Tooling print_pin = new Tooling(LayerCount, min_x, min_y, max_x, max_y, 0, 0);

            Add_Pin_pad(print_pin, ENUM_PIN_LYR.OUTTER);

            Add_L_Mark();

            Add_SM_CCD();

            Add_Slice_Hole();

            //增加 sm,ss,tent,via pin pad
            Add_Pin_pad(print_pin, ENUM_PIN_LYR.SMSSTENTVIA);

            //增加內層PIN PAD
            Add_Pin_pad(print_pin, ENUM_PIN_LYR.INNER);

            //增加drl gbx 0.125孔徑
            Add_Pin_pad(print_pin, ENUM_PIN_LYR.DRLINNGBX);

            Add_CCD_Fid_Pad();

            Add_PN_DC();

            Add_Layer_Frame();

            Add_Ten_Pad();

            Add_Tenting_Text();

            if (layers.Inner.Count > 0)
            {
                Add_Donut_Lyr_Reg();
                Add_Target_Hole();
                Add_Inn_Hole();
            }

            if (layers.SignalLayers.Count > 6 || mainWindow.need_mount_hole.IsChecked == true)
            {
                Add_Mount_Hole();
            }

            Add_Special_Symbols();
            Add_Scale_Text();

            //增加發料框
            Gen.COM($"affected_layer,name=gbx,mode=single,affected=yes");
            Gen.COM("add_polyline_strt");
            Gen.COM($"add_polyline_xy, x ={-Frame_Dist_x} ,y ={-Frame_Dist_y} ");
            Gen.COM($"add_polyline_xy, x ={-Frame_Dist_x} ,y ={Frame_y + Frame_Dist_y} ");
            Gen.COM($"add_polyline_xy, x ={Frame_x + Frame_Dist_x} ,y ={Frame_y + Frame_Dist_y} ");
            Gen.COM($"add_polyline_xy, x ={Frame_x + Frame_Dist_x} ,y ={-Frame_Dist_y} ");
            Gen.COM($"add_polyline_xy, x ={-Frame_Dist_x} ,y ={-Frame_Dist_y} ");
            Gen.COM("add_polyline_end,attributes=no,symbol=r10,polarity=positive");
            Gen.COM($"affected_layer,name=gbx,mode=single,affected=no");

            //TODO:銅面積計算改至輸出孔表時算
            /*see old panel script line# 2386
              copper_area,layer1=$cu,layer2=,drills=yes,drills_source=matrix,drills_list=drl\;2nd,resolution_value=1,thickness=$cu_thkness,x_boxes=3,y_boxes=3,area=no,dist_map=no,f_type=All,out_file=$EZCAM_DIR/tmp/123,out_layer=first
             set cop = `cat $EZCAM_DIR/tmp/123  | grep "Total copper" | cut -d ':' -f2`
            rm $EZCAM_DIR/tmp/123
            MATH cop = $cop[1] * 0.006944
                #輸出excel須要資訊
                if ($cu == $top_layer) then
                  set cop1 = $cop
                else if ($cu == $bottom_layer) then
                  set cop2 = $cop
                endif
             */

            Add_Hole_Inspection();

            Export_Drill_Table_Parameter();

            //end of script
            MessageBoxResult resultMessage = MessageBox.Show(
                "執行完畢",
                "成功結束啦..",
                MessageBoxButton.OK,
                MessageBoxImage.Asterisk);

            //Finish the script
            if (resultMessage == MessageBoxResult.OK)
                Application.Current.Shutdown(); // Close the application
        }

        //-------------------------------以下為自訂函數--------------------------------------------------------//

        /// <summary>
        ///輸出EXCEL孔表需要的資訊 Exports the drill table parameter.
        /// </summary>
        private static void Export_Drill_Table_Parameter()
        {
            //先建立臨時目錄
            string tmp_dir = "c:/tmp/drill_data";

            string user_dir = $"{GenTools.GEN_DIR}/fw/jobs/{Job}/user";
            string drill_table_info = $"{user_dir}/drill_table_info"; //儲存孔表資訊的檔案

            string exceltmpfile = $"{tmp_dir}/{Job}.exceltmpvarfile";

            //輸出截圖
            Gen.COM("clear_layers");
            Gen.COM($"zoom_area,x1={-Frame_Dist_x - 0.2},y1={-Frame_Dist_y - 0.2}" +
                    $",x2={Frame_x + Frame_Dist_x + 0.2},y2={Frame_y + Frame_Dist_y + 0.2}");
            Gen.COM("colors_change,plane=hl,color=0");
            Gen.COM("colors_change,plane=bg,color=999999");
            Gen.COM($"disp_snapshot,file={tmp_dir}/{Job}.jpg,width=0,height=0");
            Gen.COM("colors_set,mode=default");

            File.Copy($"{drill_table_info}", $"{exceltmpfile}", true);
            //File.Copy($"{tmp_dir}/{Job}.jpg",$"{user_dir}/{Job}.jpg");
        }

        /// <summary>
        /// 斷針檢驗孔 Adds the hole inspection.
        /// </summary>
        /// <exception cref="System.NotImplementedException"></exception>
        private static void Add_Hole_Inspection()
        {
            string drill_size_lyr = "find_drill_size_";
            if (GenTools.Layer_IsExist(Job, Step, $"{drill_size_lyr}"))
                Gen.COM($"delete_layer,layer={drill_size_lyr}");
            Gen.COM("affected_layer,name=,mode=all,affected=no");
            Gen.COM("clear_layers");

            //Gen.COM("affected_layer,name=drl,mode=single,affected=yes");
            Gen.COM($"flatten_layer,source_layer=drl,target_layer={drill_size_lyr}");
            Gen.COM("affected_layer,mode=all,affected=no");

            //刪除下料邊的孔,保留板內的孔
            Gen.COM($"affected_layer,name={drill_size_lyr},mode=single,affected=yes");
            Gen.COM("clip_area_strt");
            Gen.COM("clip_area_xy,x=-0.005,y=-0.005");
            Gen.COM($"clip_area_xy,x={Frame_x},y={Frame_y}");
            Gen.COM("clip_area_end" +
                    ",layers_mode=affected_layers,layer=,area=manual,area_type=rectangle" +
                    @",inout=outside,contour_cut=no,margin=0,feat_types=line\;pad\;surface\;arc\;text");

            //刪除負層(如果有的話)
            Gen.COM("filter_set,filter_name=popup,update_popup=no,polarity=negative");
            Gen.COM("filter_area_strt");
            Gen.COM("filter_area_end" +
                    ",layer=,filter_name=popup,operation=select,area_type=none" +
                    ",inside_area=no,intersect_area=no");
            Gen.COM("get_select_count");
            if (Gen.COMANS != "0")
            {
                Gen.COM("sel_delete");
            }

            //刪除所有線段
            Gen.COM(@"filter_set,filter_name=popup,update_popup=no,polarity=positive\;negative");
            Gen.COM(@"filter_set,filter_name=popup,update_popup=no,feat_types=line\;arc");
            Gen.COM("filter_area_strt");
            Gen.COM(
                @"filter_area_end,layer=,filter_name=popup,operation=select,area_type=none,inside_area=no,intersect_area=no");
            Gen.COM("get_select_count");
            if (Gen.COMANS != "0")
            {
                Gen.COM("sel_delete");
            }

            Gen.COM("filter_reset,filter_name=popup");

            string tmp_file = "C:/tmp/info_drill_list_v0.tmp";

            Gen.COM($"info,out_file={tmp_file}" +
                    $",args= -t layer -e {Job}/{Step}/{drill_size_lyr} -d FEATURES -m script,units=inch");

            SortedSet<double> drill_sorted_set = new SortedSet<double>();
            double smallest_hole = 1;

            using (StreamReader fr = new StreamReader(tmp_file))
            {
                fr.ReadLine(); //Skip first line
                while (!fr.EndOfStream)
                {
                    string input = fr.ReadLine();
                    Match match = Regex.Match(input, @"r(\d+\.?\d*)");
                    if (match.Success)
                    {
                        double value = double.Parse(match.Groups[1].Value);
                        drill_sorted_set.Add(value);

                        Match small_ = Regex.Match(input, @"drill=via");
                        if (small_.Success)
                        {
                            if (value < smallest_hole)
                                smallest_hole = value;
                        }
                    }
                }

                smallest_hole = smallest_hole > 2 ? smallest_hole : drill_sorted_set.Min;
                //drl_size = ` cat $EZCAM_DIR/tmp/hole | grep " r" | grep "#P"  | cut -d " " -f4 | cut -c2- | sort -n | uniq `
                //rm $EZCAM_DIR/tmp/hole
                //set drl_size_list = ( $drl_size_list $drl_size )
            }

            File.Delete($"{tmp_file}");
            Gen.COM($"delete_layer,layer={drill_size_lyr}");

            if (drill_sorted_set.Count == 0)
            {
                MessageBoxResult resultMessage = MessageBox.Show(
                    "未抓取到鑽孔,切片孔與斷針檢驗孔未產生",
                    "成功結束啦..",
                    MessageBoxButton.OK,
                    MessageBoxImage.Asterisk);
                return;
            }

            //斷針檢驗孔
            double drl_start_x = Min_x - 0.26;
            double drl_start_y = Min_y + 3.2;
            double space = 0.05;

            //切片孔
            double add_small_hole_x = Min_x - 0.2;
            double add_small_hole_y = Min_y + 2.3;
            double add_small_hole_xx = Max_x + 0.2;
            double add_small_hole_yy = Max_y - 2.8;
            double hole_space = 0.1;
            foreach (var drl_list in new List<string> { "drl", "gbx" })
            {
                Gen.COM($"affected_layer,name={drl_list},mode=single,affected=yes");
            }

            //加斷針檢驗孔
            double last_hole_size = 0;
            foreach (var drl_size_ in drill_sorted_set)
            {
                Gen.COM("add_pad" +
                        $",attributes=no,x={drl_start_x},y={drl_start_y}" +
                        $",symbol=r{drl_size_},polarity=positive,angle=0" +
                        $",mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
                if (last_hole_size > 0)
                {
                    drl_start_y = drl_start_y +
                                  GenTools.DoubleRound((drl_size_ + last_hole_size) / 1000 / 2) + space;
                }

                last_hole_size = drl_size_;
            }

            //加切片孔
            if (smallest_hole > 3.0)
            {
                for (int i = 0; i < 6; i++)
                {
                    Gen.COM("add_pad" +
                            $",attributes=no,x={add_small_hole_x},y={add_small_hole_y}" +
                            $",symbol=r{smallest_hole},polarity=positive,angle=0" +
                            $",mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
                    Gen.COM("add_pad" +
                            $",attributes=no,x={add_small_hole_xx},y={add_small_hole_yy}" +
                            $",symbol=r{smallest_hole},polarity=positive,angle=0" +
                            $",mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
                    add_small_hole_y += hole_space;
                    add_small_hole_yy += hole_space;
                }
            }

            Gen.COM("affected_layer,mode=all,affected=no");

            //增加切片孔Pad/Line
            double smallest_hole_pad = smallest_hole +
                                       GenTools.DoubleRound(double.Parse(mainWindow.smallest_hole_ring.Text) * 2);
            if (smallest_hole_pad > 100)
            {
                Gen.PAUSE("smallest hole pad size too big,will change to 65min");
                smallest_hole_pad = 65;
            }

            add_small_hole_x = Min_x - 0.2;
            add_small_hole_y = Min_y + 2.3;
            add_small_hole_xx = Max_x + 0.2;
            add_small_hole_yy = Max_y - 2.8;
            hole_space = 0.1;

            //加最小線寬
            double add_small_hole_x1 = Min_x - 0.2;
            double add_small_hole_y1 = Min_y + 2.3;
            double add_small_hole_x2 = Min_x - 0.2;
            double add_small_hole_y2 = Min_y + 2.4;
            double add_small_hole_xx1 = Max_x + 0.2;
            double add_small_hole_yy1 = Max_y - 2.8;
            double add_small_hole_xx2 = Max_x + 0.2;
            double add_small_hole_yy2 = Max_y - 2.7;
            double space_line = 0.4;

            foreach (var outer in layers.Outer)
            {
                Gen.COM($"affected_layer,name={outer},mode=single,affected=yes");
            }

            for (int i = 0; i < 6; i++)
            {
                Gen.COM("add_pad" +
                        $",attributes=no,x={add_small_hole_x},y={add_small_hole_y}" +
                        $",symbol=r{smallest_hole_pad},polarity=positive,angle=0" +
                        $",mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
                Gen.COM("add_pad" +
                        $",attributes=no,x={add_small_hole_xx},y={add_small_hole_yy}" +
                        $",symbol=r{smallest_hole_pad},polarity=positive,angle=0" +
                        $",mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
                add_small_hole_y += hole_space;
                add_small_hole_yy += hole_space;
            }

            //獲取最小工作線寬
            string smallest_line_width = "";
            smallest_line_width = mainWindow.outter_work_width.Text;
            //如果未輸入,預設10mil
            if (smallest_line_width == "")
                smallest_line_width = "10";
            for (int i = 0; i < 2; i++)
            {
                Gen.COM("add_line" +
                        $",attributes=no,xs={add_small_hole_x1},ys={add_small_hole_y1}" +
                        $",xe={add_small_hole_x2},ye={add_small_hole_y2}" +
                        $",symbol=r{smallest_line_width},polarity=positive");
                Gen.COM("add_line" +
                        $",attributes=no,xs={add_small_hole_xx1},ys={add_small_hole_yy1}" +
                        $",xe={add_small_hole_xx2},ye={add_small_hole_yy2}" +
                        $",symbol=r{smallest_line_width},polarity=positive");
                add_small_hole_y1 += space_line;
                add_small_hole_yy1 += space_line;
                add_small_hole_y2 += space_line;
                add_small_hole_yy2 += space_line;
            }

            Gen.COM($"affected_layer,mode=all,affected=no");
        }

        /// <summary>
        /// 增加補償字樣 Adds the scale text.
        /// </summary>
        private static void Add_Scale_Text()
        {
            if (layers.Via.Count > 0)
            {
                foreach (var via in layers.Via)
                {
                    Gen.COM($"affected_layer,name={via},mode=single,affected=yes");
                    string via_scale_text = "X:0.9992 Y:0.9996";
                    double via_text_len = GenTools.DoubleRound(via_scale_text.Length * 0.09);
                    double text_loc_x = Max_x + 0.25;
                    double text_loc_y = Min_y + 4.9 + via_text_len;
                    string ang = "";
                    string mir = "";

                    if (via == "v1")
                    {
                        ang = "270";
                        mir = "no";
                        text_loc_y = Min_y + 4.9 + via_text_len;
                    }
                    else
                    {
                        ang = "90";
                        mir = "yes";
                        text_loc_y = Min_y + 4.9 + GenTools.DoubleRound(via_text_len * 2);
                    }
                    //負擋漲縮值

                    Gen.COM("add_text" +
                            $",attributes=no,type=string,x={text_loc_x},y={text_loc_y}" +
                            $",text={via_scale_text},x_size=0.09,y_size=0.09" +
                            $",w_factor=0.833333333333333,polarity=positive,angle={ang}" +
                            $",mirror={mir},fontname=standard,ver=1");

                    Gen.COM($"affected_layer,name={via},mode=single,affected=no");
                }
            }

            if (layers.SilkScreen.Count > 0)
            {
                foreach (var ss in layers.SilkScreen)
                {
                    Gen.COM($"affected_layer,name={ss},mode=single,affected=yes");
                    string silk_scale_text = "X:0.9996 Y:0.9997";
                    double silk_text_len = GenTools.DoubleRound(silk_scale_text.Length * 0.09);
                    double text_loc_x = Min_x + 0.7;
                    double text_loc_y = Max_y + 0.26;
                    string mir = "";

                    if (layers.Full_Layer_Side[ss] == "top")
                    {
                        mir = "no";
                        text_loc_x = Min_x + 0.7;
                    }
                    else if (layers.Full_Layer_Side[ss] == "bottom")
                    {
                        mir = "yes";
                        text_loc_x += silk_text_len;
                    }
                    //負擋漲縮值

                    Gen.COM("add_text" +
                            $",attributes=no,type=string,x={text_loc_x},y={text_loc_y}" +
                            $",text={silk_scale_text},x_size=0.09,y_size=0.09" +
                            $",w_factor=0.833333333333333,polarity=positive,angle=0" +
                            $",mirror={mir},fontname=standard,ver=1");

                    Gen.COM($"affected_layer,name={ss},mode=single,affected=no");
                }
            }
        }

        /// <summary>
        /// 板邊加特殊符號 Adds the special symbols.
        /// </summary>
        private static void Add_Special_Symbols()
        {
            //min solder sam
            if (layers.SolderMask.Count > 0)
            {
                foreach (string sm in layers.SolderMask)
                {
                    Gen.COM($"affected_layer,name={sm},mode=single,affected=yes");
                    double dam_text_x = max_x + 0.25;
                    double dam_text_y = min_y + 5.5;
                    string min_dam_size = mainWindow.min_dam_size.Text;
                    if (min_dam_size == null || min_dam_size == "") min_dam_size = "4";
                    double sym_size = 0.37;
                    double dam_text_size = GenTools.DoubleRound(min_dam_size.Length * 0.09);
                    if (layers.Full_Layer_Side[sm] == "top")
                    {
                        Gen.COM("add_pad" +
                                ",attributes=no" +
                                $",x={dam_text_x},y={dam_text_y},symbol={sym.Pnl_Min_Dam}" +
                                $",polarity=positive,angle=0,mirror=no,nx=1,ny=1,dx=0,dy=0" +
                                $",xscale=1,yscale=1");
                        double no_mir_text = GenTools.DoubleRound(dam_text_y + 0.45);
                        dam_text_x += GenTools.DoubleRound(sym_size / 2 - 0.12);
                        Gen.COM("add_text" +
                                $",attributes=no,type=string,x={dam_text_x},y={no_mir_text}" +
                                $",text={min_dam_size}mil,x_size=0.12,y_size=0.12,w_factor=0.833333333333333" +
                                $",polarity=positive,angle=270,mirror=no,fontname=standard");
                    }
                    else if (layers.Full_Layer_Side[sm] == "bottom")
                    {
                        dam_text_y = min_y + 6;
                        Gen.COM("add_pad" +
                                ",attributes=no" +
                                $",x={dam_text_x},y={dam_text_y},symbol={sym.Pnl_Min_Dam}" +
                                $",polarity=positive,angle=180,mirror=yes,nx=1,ny=1,dx=0,dy=0" +
                                $",xscale=1,yscale=1");
                        double mir_text = GenTools.DoubleRound(dam_text_y - 0.45);
                        dam_text_x += GenTools.DoubleRound(sym_size / 2 - 0.12);
                        Gen.COM("add_text" +
                                $",attributes=no,type=string,x={dam_text_x},y={mir_text}" +
                                $",text={min_dam_size}mil,x_size=0.12,y_size=0.12,w_factor=0.833333333333333" +
                                $",polarity=positive,angle=90,mirror=yes,fontname=standard");
                    }
                    //增加防焊CCD框

                    double sm_ccd_frame = Min_x - 1.0394;
                    double sm_ccd_loc_y = GenTools.DoubleRound(-(27.4 - (Max_y - Min_y)) / 2);

                    Gen.COM("add_pad" +
                            $",attributes=no,x={sm_ccd_frame},y={sm_ccd_loc_y},symbol={sym.Pnl_Sm_Ccd_Outline}" +
                            $",polarity=positive,angle=0,mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");

                    Gen.COM($"affected_layer,name={sm},mode=single,affected=no");
                }
            }

            //增加Dte Code
            if (mainWindow.dc_lyr_sel.SelectedIndex == -1)
            {
                MessageBoxResult resultMessage = MessageBox.Show(
                    "未選擇週期層別,麻煩手動添加",
                    "週期層別未選擇",
                    MessageBoxButton.OK,
                    MessageBoxImage.Question);
            }
            else
            {
                string dc_lyr = mainWindow.dc_lyr_sel.Text;
                string dc_sym = "";
                if (mainWindow.dc_yyww.IsChecked == true)
                {
                    dc_sym = $"{sym.Pnl_Dc_Yyww}";
                }
                else if (mainWindow.dc_wwyy.IsChecked == true)
                {
                    dc_sym = $"{sym.Pnl_Dc_Wwyy}";
                }

                double dc_loc_x = Max_x + 0.35;
                double dc_loc_y = Min_y + 5;
                string pos_neg = "";
                string ang = "";
                string mir = "";
                if (layers.Outer.Contains(dc_lyr))
                {
                    pos_neg = "negative";
                }
                else
                {
                    pos_neg = "positive";
                }

                if (layers.Full_Layer_Side[dc_lyr] == "top")
                {
                    ang = "0";
                    mir = "no";
                }
                else if (layers.Full_Layer_Side[dc_lyr] == "bottom")
                {
                    ang = "180";
                    mir = "yes";
                }

                Gen.COM($"affected_layer,name={dc_lyr},mode=single,affected=yes");
                Gen.COM("add_pad" +
                        $",attributes=no,x={dc_loc_x},y={dc_loc_y},symbol={dc_sym}" +
                        $",polarity={pos_neg},angle={ang},mirror={mir}" +
                        $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
                Gen.COM($"affected_layer,name={dc_lyr},mode=single,affected=no");
            }

            //增加負擋片symbol
            if (layers.Via.Count == 1 && layers.Via[0] == "v1")
            {
                Gen.COM($"affected_layer,name={layers.Via[0]},mode=single,affected=yes");
                double via_text_x = max_x + 0.25;
                double via_text_y = min_y + 5.5;
                Gen.COM("add_pad" +
                        ",attributes=no" +
                        $",x={via_text_x},y={via_text_y},symbol={sym.Pnl_Both_Tenting}" +
                        $",polarity=positive,angle=0,mirror=no,nx=1,ny=1,dx=0,dy=0" +
                        $",xscale=1,yscale=1");

                Gen.COM($"affected_layer,name={layers.Via[0]},mode=single,affected=no");
            }
            else
            {
                string cs = "";
                foreach (var lyr in layers.Via)
                {
                    cs = lyr == "v1" ? "c" : "s";

                    double via_text_x = max_x + 0.285;
                    double via_text_y = min_y + 5.5;
                    double sym_size = 0.37;
                    double via_text_size = 0.12;
                    string via_text = "";
                    string mir = "";
                    string text_ang = "";
                    string sym_ang = "";
                    double no_mir_text = 0;

                    Gen.COM($"affected_layer,name={lyr},mode=single,affected=yes");

                    if (cs == "c")
                    {
                        mir = "no";
                        text_ang = "270";
                        sym_ang = "0";
                    }
                    else if (cs == "s")
                    {
                        mir = "yes";
                        text_ang = "90";
                        sym_ang = "180";
                    }

                    Gen.COM("add_pad" +
                            ",attributes=no" +
                            $",x={via_text_x},y={via_text_y},symbol={sym.Pnl_Via}" +
                            $",polarity=positive,angle={sym_ang},mirror={mir},nx=1,ny=1,dx=0,dy=0" +
                            $",xscale=1,yscale=1");

                    if (cs == "c")
                    {
                        via_text = "C";
                        via_text_x += GenTools.DoubleRound(sym_size / 2 - 0.1);
                        no_mir_text = GenTools.DoubleRound(via_text_y + via_text_size - 0.7);
                    }
                    else if (cs == "s")
                    {
                        via_text = "S";
                        via_text_x += GenTools.DoubleRound(sym_size / 2 - 0.1);
                        no_mir_text = GenTools.DoubleRound(via_text_y - via_text_size + 0.7);
                    }

                    Gen.COM("add_text" +
                            $",attributes=no,type=string,x={via_text_x},y={no_mir_text}" +
                            $",text={via_text},x_size=0.15,y_size=0.15,w_factor=0.833333333333333" +
                            $",polarity=positive,angle={text_ang},mirror={mir},fontname=standard");
                    Gen.COM($"affected_layer,name={lyr},mode=single,affected=no");
                }
            }

            //增加塞孔片symbol
            if (layers.Tent.Count > 0)
            {
                string cs = "";
                foreach (var lyr in layers.Tent)
                {
                    cs = lyr == "t1" ? "c" : "s";

                    double tent_text_x = max_x + 0.285;
                    double tent_text_y = min_y + 5.5;
                    double sym_size = 0.37;
                    double tent_text_size = 0.12;
                    string tent_text = "";
                    string mir = "";
                    string text_ang = "";
                    string sym_ang = "";
                    double no_mir_text = 0;

                    Gen.COM($"affected_layer,name={lyr},mode=single,affected=yes");

                    if (cs == "c")
                    {
                        mir = "no";
                        text_ang = "270";
                        sym_ang = "0";
                    }
                    else if (cs == "s")
                    {
                        mir = "yes";
                        text_ang = "90";
                        sym_ang = "180";
                    }

                    Gen.COM("add_pad" +
                            ",attributes=no" +
                            $",x={tent_text_x},y={tent_text_y},symbol={sym.Pnl_Tent}" +
                            $",polarity=positive,angle={sym_ang},mirror={mir},nx=1,ny=1,dx=0,dy=0" +
                            $",xscale=1,yscale=1");

                    if (cs == "c")
                    {
                        tent_text = "C";
                        tent_text_x += GenTools.DoubleRound(sym_size / 2 - 0.1);
                        no_mir_text = GenTools.DoubleRound(tent_text_y + tent_text_size - 0.7);
                    }
                    else if (cs == "s")
                    {
                        tent_text = "S";
                        tent_text_x += GenTools.DoubleRound(sym_size / 2 - 0.1);
                        no_mir_text = GenTools.DoubleRound(tent_text_y - tent_text_size + 0.7);
                    }

                    Gen.COM("add_text" +
                            $",attributes=no,type=string,x={tent_text_x},y={no_mir_text}" +
                            $",text={tent_text},x_size=0.15,y_size=0.15,w_factor=0.833333333333333" +
                            $",polarity=positive,angle={text_ang},mirror={mir},fontname=standard");
                    Gen.COM($"affected_layer,name={lyr},mode=single,affected=no");
                }
            }

            //增加負片擋點symnool
        }

        /// <summary>
        /// 切片孔/PAD
        /// </summary>
        private static void Add_Slice_Hole()
        {
            string sym_ = "";
            for (int i = 0; i < 2; i++)
            {
                if (i == 0)
                {
                    sym_ = $"{sym.Pnl_Mic_N}";
                    foreach (string outlayer_ in layers.Outer)
                    {
                        Gen.COM($"affected_layer,name={outlayer_},mode=single,affected=yes");
                    }
                }
                else if (i == 1)
                {
                    sym_ = $"{sym.Pnl_Mic_Inn}";
                    if (layers.Inner.Count > 0)
                    {
                        //增加內層切片孔PAS
                        foreach (string inner in layers.Inner)
                        {
                            Gen.COM($"affected_layer,name={inner},mode=single,affected=yes");
                        }
                    }
                }

                //增加切片孔PAS
                Gen.COM("add_pad" +
                        ",attributes=no" +
                        $",x={min_x - 0.2},y={min_y + 2.55},symbol={sym_}" +
                        $",polarity=positive,angle=0,mirror=no,nx=1,ny=1,dx=0,dy=0" +
                        $",xscale=1,yscale=1");

                Gen.COM("add_pad" +
                        ",attributes=no" +
                        $",x={max_x + 0.2},y={max_y - 2.55},symbol={sym_}" +
                        $",polarity=positive,angle=0,mirror=no,nx=1,ny=1,dx=0,dy=0" +
                        $",xscale=1,yscale=1");

                GenTools.Clear_Layers();
            }
        }

        /// <summary>
        /// 增加Inn孔/Pad
        /// </summary>
        private static void Add_Inn_Hole()
        {
            for (int i = 0; i < 2; i++)
            {
                double symbol_inn_x1 = min_x - 0.2;
                double symbol_inn_y1 = min_y + 5;
                double symbol_inn_x2 = max_x + 0.2;
                double symbol_inn_y2 = min_y + 5;
                double symbol_inn_x3 = min_x + 2.5;
                double symbol_inn_y3 = min_y - 0.2;
                double symbol_inn_x4 = min_x + 2.5;
                double symbol_inn_y4 = max_y + 0.2;

                double limit_x = Frame_x + Frame_Dist_x;
                double limit_y = Frame_y + Frame_Dist_y;
                string inn_sym = "";
                if (i == 0) //增加INN PAD
                {
                    inn_sym = $"{sym.Pnl_Inn_Pad}";
                    foreach (string inner in layers.Inner)
                    {
                        Gen.COM($"affected_layer,name={inner},mode=single,affected=yes");
                    }
                }
                else if (i == 1)
                {
                    inn_sym = "r125";
                    Gen.COM($"affected_layer,name=inn,mode=single,affected=yes");
                    Gen.COM($"affected_layer,name=gbx,mode=single,affected=yes");
                }

                while (limit_y > symbol_inn_y1 || limit_x > symbol_inn_x3)
                {
                    if (limit_y > symbol_inn_y1)
                    {
                        Gen.COM("add_pad" +
                                $",attributes=no,x={symbol_inn_x1},y={symbol_inn_y1}" +
                                $",symbol={inn_sym},polarity=positive,angle=0,mirror=no" +
                                $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
                        Gen.COM("add_pad" +
                                $",attributes=no,x={symbol_inn_x2},y={symbol_inn_y2}" +
                                $",symbol={inn_sym},polarity=positive,angle=0,mirror=no" +
                                $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");

                        symbol_inn_y1 += 2.5;
                        symbol_inn_y2 += 2.5;
                    }

                    if (limit_x > symbol_inn_x3)
                    {
                        Gen.COM("add_pad" +
                                $",attributes=no,x={symbol_inn_x3},y={symbol_inn_y3}" +
                                $",symbol={inn_sym},polarity=positive,angle=0,mirror=no" +
                                $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
                        Gen.COM("add_pad" +
                                $",attributes=no,x={symbol_inn_x4},y={symbol_inn_y4}" +
                                $",symbol={inn_sym},polarity=positive,angle=0,mirror=no" +
                                $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
                        symbol_inn_x3 += 2.5;
                        symbol_inn_x4 += 2.5;
                    }
                }

                GenTools.Clear_Layers();
            }
        }

        /// <summary>
        /// 增加靶孔/pad
        /// </summary>
        private static void Add_Target_Hole()
        {
            double add_tag_x1 = min_x + 1.2;
            double add_tag_x2 = max_x - 1.2;
            double add_tag_x3 = max_x - 2.2;
            double add_tag_y1 = min_y - 0.3;
            double add_tag_y2 = max_y + 0.3;
            List<double> add_tag_x = new List<double>()
            {
                add_tag_x1, add_tag_x2,
                add_tag_x3, add_tag_x1,
            };
            List<double> add_tag_y = new List<double>()
            {
                add_tag_y1, add_tag_y1,
                add_tag_y2, add_tag_y2,
            };

            //增加內層P靶孔AD(pin,gbx)
            foreach (string inner in layers.InnerList)
            {
                Gen.COM($"affected_layer,name={inner},mode=single,affected=yes");
            }

            for (int i = 0; i < add_tag_x.Count; i++)
            {
                Gen.COM("add_pad" +
                        $",attributes=no,x={add_tag_x[i]},y={add_tag_y[i]}" +
                        $",symbol=s590,polarity=negative,angle=0,mirror=no" +
                        $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
                Gen.COM("add_pad" +
                        $",attributes=no,x={add_tag_x[i]},y={add_tag_y[i]}" +
                        $",symbol={sym.Pnl_Tag},polarity=positive,angle=0,mirror=no" +
                        $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
            }

            Gen.COM($"affected_layer,mode=all,affected=no");

            //增加靶孔

            Gen.COM($"affected_layer,name=pin,mode=single,affected=yes");
            Gen.COM($"affected_layer,name=gbx,mode=single,affected=yes");
            for (int i = 0; i < add_tag_x.Count; i++)
            {
                Gen.COM("add_pad" +
                        $",attributes=no,x={add_tag_x[i]},y={add_tag_y[i]}" +
                        $",symbol=r125,polarity=positive,angle=0,mirror=no" +
                        $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
            }

            Gen.COM($"affected_layer,mode=all,affected=no");
        }

        /// <summary>
        /// 增加鉚釘孔/pad
        /// </summary>
        private static void Add_Mount_Hole()
        {
            double mnt_pad_x1 = min_x - 0.35;
            double mnt_pad_x2 = max_x + 0.35;
            double mnt_pad_x3 = Max_x - 4;
            double mnt_pad_x4 = Min_x + 4;

            double mnt_pad_y1 = (max_y - min_y) / 2 - 0.1;
            double mnt_pad_y2 = Frame_y + Frame_Dist_y - 1.5;
            double mnt_pad_y3 = -Frame_Dist_y + 1.75;
            double mnt_pad_y4 = (max_y - min_y) / 2 - 0.6;
            double mnt_pad_y5 = -Frame_Dist_y + 1.75;
            double mnt_pad_y6 = max_y + 0.35;
            double mnt_pad_y7 = min_y - 0.35;
            List<double> mnt_pad_list_x = new List<double>()
            {
                mnt_pad_x1, mnt_pad_x1,
                mnt_pad_x1, mnt_pad_x2,
                mnt_pad_x2, mnt_pad_x2,
                mnt_pad_x3, mnt_pad_x3,
                mnt_pad_x4, mnt_pad_x4,
            };
            List<double> mnt_pad_list_y = new List<double>()
            {
                mnt_pad_y1, mnt_pad_y2,
                mnt_pad_y3, mnt_pad_y4,
                mnt_pad_y2, mnt_pad_y5,
                mnt_pad_y6, mnt_pad_y7,
                mnt_pad_y6, mnt_pad_y7,
            };
            List<int> mnt_dr_ang_list = new List<int>()
            {
                0, 0,
                0, 180,
                180, 180,
                90, 270,
                90, 270,
            };
            foreach (string inner_ in layers.InnerList)
            {
                Gen.COM($"affected_layer,name={inner_},mode=single,affected=yes");
            }

            string sym_ = "";
            //增加鉚釘孔pad
            for (int i = 0; i < mnt_pad_list_x.Count; i++)
            {
                Gen.COM("add_pad" +
                        $",attributes=no,x={mnt_pad_list_x[i]},y={mnt_pad_list_y[i]}" +
                        $",symbol={sym.Pnl_Inn_Mnt},polarity=positive,angle=0,mirror=no" +
                        $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
                if (i == 0 || i == 3)
                {
                    sym_ = $"{sym.Pnl_Inn_Mnt_Dr2}";
                }
                else
                {
                    sym_ = $"{sym.Pnl_Inn_Mnt_Dr}";
                }

                //增加內層排屑孔clearance
                Gen.COM("add_pad" +
                        $",attributes=no,x={mnt_pad_list_x[i]},y={mnt_pad_list_y[i]}" +
                        $",symbol={sym_},polarity=negative,angle={mnt_dr_ang_list[i]}" +
                        $",mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
            }

            GenTools.Clear_Layers();

            //增加外層排屑孔clearance
            foreach (string outer_ in layers.Outer)
            {
                Gen.COM($"affected_layer,name={outer_},mode=single,affected=yes");
            }

            for (int i = 0; i < mnt_pad_list_x.Count; i++)
            {
                string sym_dr = "";
                if (i == 0 || i == 3)
                {
                    sym_dr = $"{sym.Pnl_Inn_Mnt_Dr2_N}";
                }
                else
                {
                    sym_dr = $"{sym.Pnl_Inn_Mnt_Dr_N}";
                }

                Gen.COM("add_pad" +
                        $",attributes=no,x={mnt_pad_list_x[i]},y={mnt_pad_list_y[i]}" +
                        $",symbol={sym_dr},polarity=negative,angle={mnt_dr_ang_list[i]}" +
                        $",mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
            }

            GenTools.Clear_Layers();

            //增加鉚釘孔(drl,gbx)

            Gen.COM($"affected_layer,name=drl,mode=single,affected=yes");
            Gen.COM($"affected_layer,name=gbx,mode=single,affected=yes");

            for (int i = 0; i < mnt_pad_list_x.Count; i++)
            {
                Gen.COM("add_pad" +
                        $",attributes=no,x={mnt_pad_list_x[i]},y={mnt_pad_list_y[i]}" +
                        $",symbol=r125,polarity=positive,angle=0,mirror=no" +
                        $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
            }

            Gen.COM($"affected_layer,name=drl,mode=single,affected=no");
            Gen.COM($"affected_layer,name=gbx,mode=single,affected=no");

            //增加排氣孔

            Gen.COM($"affected_layer,name=inn,mode=single,affected=yes");
            Gen.COM($"affected_layer,name=gbx,mode=single,affected=yes");

            string sym_2 = "";
            for (int i = 0; i < mnt_pad_list_x.Count; i++)
            {
                if (i == 0 || i == 3)
                {
                    sym_2 = $"{sym.Pnl_Inn_Mnt_Dr2}";
                }
                else
                {
                    sym_2 = $"{sym.Pnl_Inn_Mnt_Dr}";
                }

                Gen.COM("add_pad" +
                        $",attributes=no,x={mnt_pad_list_x[i]},y={mnt_pad_list_y[i]}" +
                        $",symbol={sym_2},polarity=positive,angle={mnt_dr_ang_list[i]},mirror=no" +
                        $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
            }

            Gen.COM($"affected_layer,name=inn,mode=single,affected=no");
            Gen.COM($"affected_layer,name=gbx,mode=single,affected=no");
        }

        /// <summary>
        /// 層間對準甜甜圈Adds the dount lyr reg.
        /// </summary>
        private static void Add_Donut_Lyr_Reg()
        {
            double add_symbol_donut_x1 = GenTools.DoubleRound(-Frame_Dist_x + (frame_dist_x - Math.Abs(Min_x)) / 2);
            double add_symbol_donut_x2 = GenTools.DoubleRound(Max_x + (Frame_x + Frame_Dist_x - Max_x) / 2);
            double add_symbol_donut_y1 = GenTools.DoubleRound(-Frame_Dist_y + 0.3);
            double add_symbol_donut_y2 = GenTools.DoubleRound(Frame_y + Frame_Dist_y - 0.3);
            //double f_x = (Wix - max_x - min_x) / 2;
            //double f_y = (Wix - max_x - min_x) / 2;
            //double add_symbol_donut_x1 = GenTools.DoubleRound(min_x - f_x / 4);
            //double add_symbol_donut_x2 = GenTools.DoubleRound(max_x + f_x / 4);
            //double add_symbol_donut_y1 = GenTools.DoubleRound(min_y - f_y / 2 + 0.3);
            //double add_symbol_donut_y2 = GenTools.DoubleRound(max_y + f_y / 2 - 0.3);

            //double add_symbol_donut_x1 = GenTools.DoubleRound(-frame_dist_x + frame_dist_x / 4);
            //double add_symbol_donut_x2 = GenTools.DoubleRound(Six + frame_dist_x / 4);
            //double add_symbol_donut_y1 = GenTools.DoubleRound(-frame_dist_y + (frame_dist_y / 2) + 0.3);
            //double add_symbol_donut_y2 = GenTools.DoubleRound(Siy + frame_dist_y / 2 - 0.3);
            List<double> sym_dount_x = new List<double>()
            {
                add_symbol_donut_x1, add_symbol_donut_x1,
                add_symbol_donut_x2, add_symbol_donut_x2,
            };

            List<double> sym_dount_y = new List<double>()
            {
                add_symbol_donut_y1, add_symbol_donut_y2,
                add_symbol_donut_y1, add_symbol_donut_y2,
            };

            int d_num = 1;
            int symbol_donut_add = 0;
            int symbol_donut_width = 0;
            int symbol_donut_height = 0;
            List<string> symbol_dounut_list = new List<string>();

            foreach (string inner in layers.Inner)
            {
                symbol_donut_add = (d_num - 1) * 25;
                symbol_donut_width = 80 + symbol_donut_add;
                symbol_donut_height = 60 + symbol_donut_add;

                symbol_dounut_list.Add(
                    $"{symbol_donut_width}x{symbol_donut_height}");
                d_num++;
            }

            symbol_donut_width += 5;
            int symbol_outer = symbol_donut_width;
            symbol_donut_width += 15;
            int symbol_biggest = symbol_donut_width;

            for (int i = 0; i < layers.Inner.Count; i++)
            {
                Gen.COM($"affected_layer,name={layers.Inner[i]},mode=single,affected=yes");
                for (int j = 0; j < sym_dount_x.Count; j++)
                {
                    Gen.COM(("add_pad" +
                             $",attributes=no,x={sym_dount_x[j]},y={sym_dount_y[j]}" +
                             $",symbol=r{symbol_biggest},polarity=negative" +
                             $",angle=0,mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1"));
                    Gen.COM(("add_pad" +
                             $",attributes=no,x={sym_dount_x[j]},y={sym_dount_y[j]}" +
                             $",symbol=donut_r{symbol_dounut_list[i]},polarity=positive" +
                             $",angle=0,mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1"));
                }

                Gen.COM($"affected_layer,name={layers.Inner[i]},mode=single,affected=no");
            }

            //外層donut
            symbol_donut_width += 25;
            symbol_donut_height += 25;
            foreach (string outer in layers.Outer)
            {
                Gen.COM($"affected_layer,name={outer},mode=single,affected=yes");
            }

            if (layers.SignalEmpty.Count > 0)
            {
                foreach (string empty_lyr in layers.SignalEmpty)
                {
                    Gen.COM($"affected_layer,name={empty_lyr},mode=single,affected=yes");
                }
            }

            for (int i = 0; i < sym_dount_x.Count; i++)
            {
                Gen.COM(("add_pad" +
                         $",attributes=no,x={sym_dount_x[i]},y={sym_dount_y[i]}" +
                         $",symbol=r{symbol_outer},polarity=negative" +
                         $",angle=0,mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1"));
            }

            GenTools.Clear_Layers();
        }

        /// <summary>
        ///增加外層負片字樣 Adds the tenting text.
        /// </summary>
        private static void Add_Tenting_Text()
        {
            if (mainWindow.outter_neg.IsChecked == true)
            {
                double add_tenting_x = max_x + 0.38;
                double add_tenting_y = min_y + 6.4;
                int tnt_ang = 0;
                string tnt_mir = "no";
                if (layers.Outer.Count > 0)
                {
                    foreach (string outerLyr in layers.Outer)
                    {
                        Gen.COM($"affected_layer,name={outerLyr},mode=single,affected=yes");
                        if (layers.Full_Layer_Side[outerLyr] == "top")
                        {
                            tnt_ang = 0;
                            tnt_mir = "no";
                        }
                        else if (layers.Full_Layer_Side[outerLyr] == "bottom")
                        {
                            tnt_ang = 180;
                            tnt_mir = "yes";
                        }

                        Gen.COM("add_pad" +
                                $",attributes=no,x={add_tenting_x},y={add_tenting_y}" +
                                $",symbol={sym.Pnl_Tenting_Process},polarity=negative" +
                                $",angle={tnt_ang},mirror={tnt_mir},nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");

                        Gen.COM($"affected_layer,name={outerLyr},mode=single,affected=no");
                    }
                }
            }
        }

        /// <summary>
        /// 增加層別顯示框 Adds the layer frame.
        /// </summary>
        private static void Add_Layer_Frame()
        {
            double layer_frame_x = max_x + 0.3; //層別框架X
            double layer_frame_y = GenTools.DoubleRound((max_y - min_y) / 2 + 0.585); //層別框架Y

            double layer_frame_name_x = layer_frame_x - 0.073728; //層別名稱X
            double layer_frame_name_y = layer_frame_y + 0.0576; //層別名稱X

            double frame_width = 0.1152;

            string sym_ = "";
            int inner_cnt = 1;
            string mir_lyr_name = "no";
            int ang_lyr_name = 0;
            foreach (var lyr in layers.SignalLayers)
            {
                Gen.COM($"affected_layer,name={lyr},mode=single,affected=yes");
                if (layers.Outer.Contains(lyr)) //外層
                {
                    if (layers.Full_Layer_Side[lyr] == "top")
                    {
                        sym_ = $"{MySymbols.Pnl_Layer_Name_List[0]}";
                        mir_lyr_name = "no";
                        ang_lyr_name = 0;
                    }
                    else if (layers.Full_Layer_Side[lyr] == "bottom")
                    {
                        sym_ = $"{MySymbols.Pnl_Layer_Name_List[MySymbols.Pnl_Layer_Name_List.Length - 1]}";
                        mir_lyr_name = "yes";
                        ang_lyr_name = 180;
                    }
                }
                else if (layers.Inner.Contains(lyr)) //內層
                {
                    if (inner_cnt < 13)
                        sym_ = $"{MySymbols.Pnl_Layer_Name_List[inner_cnt++]}";
                }

                if (layers.Full_Layer_Side[lyr] == "top")
                {
                    mir_lyr_name = "no";
                    ang_lyr_name = 0;
                }
                else if (layers.Full_Layer_Side[lyr] == "bottom")
                {
                    mir_lyr_name = "yes";
                    ang_lyr_name = 180;
                }

                Gen.COM("add_pad" +
                        ",attributes=no" +
                        $",x={layer_frame_x},y={layer_frame_y}" +
                        $",symbol={sym.Pnl_Layer_Text_Frame}" +
                        $",polarity=positive,angle=0,mirror=no" +
                        $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");

                if (inner_cnt < 13)
                {
                    Gen.COM("add_pad" +
                            $",attributes=no,x={layer_frame_name_x},y={layer_frame_name_y}" +
                            $",symbol={sym_},polarity=positive,angle={ang_lyr_name},mirror={mir_lyr_name}" +
                            $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
                }

                layer_frame_name_y += frame_width;
                Gen.COM($"affected_layer,name={lyr},mode=single,affected=no");
            }
        }

        /// <summary>
        ///板邊增加廠料 Adds the pn dc.
        /// </summary>
        private static void Add_PN_DC()
        {
            List<List<string>> LayerList = new List<List<string>>()
            {
                layers.Outer, layers.SolderMask, layers.SilkScreen, new List<string>() { "gbx" },
                layers.InnerList
            };
            for (int i = 0; i < LayerList.Count; i++)
            {
                double pn_dc_clearance_box_x = max_x + 0.215; //廠料內縮
                double pn_dc_clearance_box_y = min_y + 2.47; //廠料內縮

                double add_job_date_x = max_x + 0.25;
                double add_job_date_y = min_y + 2.1;

                double x_size = 0.09; //廠料字體大小
                double y_size = 0.09; //廠料字體大小
                double w_factor = 1.25; //廠料字體寬度因子
                //double text_loc_x = GenTools.DoubleRound(x_size / 2);
                //double text_loc_y = GenTools.DoubleRound(y_size / 2);

                double text_angle = 270; //廠料文字角度
                string mirror_text = "no"; //廠料文字鏡像

                List<string> text_lalyer = LayerList[i];

                switch (i)
                {
                    // 0: outer layer
                    case 0:
                        if (text_lalyer.Count > 0)
                        {
                            foreach (string outer in layers.Outer)
                            {
                                string lyr_text = "";
                                Gen.COM($"affected_layer,name={outer},mode=single,affected=yes");
                                if (layers.Full_Layer_Side[outer] == "top")
                                {
                                    text_angle = 270;
                                    mirror_text = "no"; //廠料文字鏡像
                                    add_job_date_y = min_y + 2.1;
                                    lyr_text = "$$JOB TOP $$DATE-MMDDYY";
                                }
                                else if (layers.Full_Layer_Side[outer] == "bottom")
                                {
                                    text_angle = 90;
                                    mirror_text = "yes"; //廠料文字鏡像
                                    lyr_text = "$$JOB BOT $$DATE-MMDDYY";
                                    double text_len = lyr_text.Length; //廠料文字長度
                                    text_len = GenTools.DoubleRound(text_len * 0.09);
                                    add_job_date_y = min_y + 1.985 + text_len;
                                }

                                Gen.COM("add_pad" +
                                        $",attributes=no,x={pn_dc_clearance_box_x},y={pn_dc_clearance_box_y + 0.7}" +
                                        $",symbol=rect189x2252,polarity={GenTools.POS_NEG["P"]},angle=0,mirror=no" +
                                        $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");

                                Gen.COM("add_text" +
                                        ",attributes=no,type=string" +
                                        $",x={add_job_date_x},y={add_job_date_y}" +
                                        $",text={lyr_text},x_size={x_size},y_size={y_size}" +
                                        $",w_factor={w_factor}" +
                                        $",polarity={GenTools.POS_NEG["N"]}" +
                                        $",angle={text_angle},mirror={mirror_text},fontname=standard,ver=1");

                                Gen.COM($"affected_layer,name={outer},mode=single,affected=no");
                            }
                        }

                        break;
                    // 1: solder mask layer
                    case 1:
                        if (text_lalyer.Count > 0)
                        {
                            foreach (string sm in layers.SolderMask)
                            {
                                string lyr_text = "";
                                Gen.COM($"affected_layer,name={sm},mode=single,affected=yes");
                                if (layers.Full_Layer_Side[sm] == "top")
                                {
                                    text_angle = 270;
                                    mirror_text = "no"; //廠料文字鏡像
                                    add_job_date_y = min_y + 2.1;
                                    lyr_text = @"$$JOB TOP MASK $$DATE-MMDDYY";
                                }
                                else if (layers.Full_Layer_Side[sm] == "bottom")
                                {
                                    text_angle = 90;
                                    mirror_text = "yes"; //廠料文字鏡像
                                    lyr_text = @"$$JOB BOT MASK $$DATE-MMDDYY";
                                    double text_len = lyr_text.Length; //廠料文字長度
                                    text_len = GenTools.DoubleRound(text_len * 0.09);
                                    add_job_date_y = min_y + 1.985 + text_len;
                                }

                                Gen.COM("add_text" +
                                        ",attributes=no,type=string" +
                                        $",x={add_job_date_x},y={add_job_date_y}" +
                                        $",text={lyr_text},x_size={x_size},y_size={y_size}" +
                                        $",w_factor={w_factor}" +
                                        $",polarity={GenTools.POS_NEG["P"]}" +
                                        $",angle={text_angle},mirror={mirror_text},fontname=standard,ver=1");

                                Gen.COM($"affected_layer,name={sm},mode=single,affected=no");
                            }
                        }

                        break;
                    // 2: silk screen layer
                    case 2:
                        if (text_lalyer.Count > 0)
                        {
                            foreach (string ss in layers.SilkScreen)
                            {
                                string lyr_text = "";
                                Gen.COM($"affected_layer,name={ss},mode=single,affected=yes");
                                if (layers.Full_Layer_Side[ss] == "top")
                                {
                                    text_angle = 270;
                                    mirror_text = "no"; //廠料文字鏡像
                                    add_job_date_y = min_y + 2.1;
                                    lyr_text = @"$$JOB TOP SILK $$DATE-MMDDYY";
                                }
                                else if (layers.Full_Layer_Side[ss] == "bottom")
                                {
                                    text_angle = 90;
                                    mirror_text = "yes"; //廠料文字鏡像
                                    lyr_text = @"$$JOB BOT SILK $$DATE-MMDDYY";
                                    double text_len = lyr_text.Length; //廠料文字長度
                                    text_len = GenTools.DoubleRound(text_len * 0.09);
                                    add_job_date_y = min_y + 1.985 + text_len;
                                }

                                Gen.COM("add_text" +
                                        ",attributes=no,type=string" +
                                        $",x={add_job_date_x},y={add_job_date_y}" +
                                        $",text={lyr_text},x_size={x_size},y_size={y_size}" +
                                        $",w_factor={w_factor}" +
                                        $",polarity={GenTools.POS_NEG["P"]}" +
                                        $",angle={text_angle},mirror={mirror_text},fontname=standard,ver=1");

                                Gen.COM($"affected_layer,name={ss},mode=single,affected=no");
                            }
                        }

                        break;
                    // 3: gbx layer
                    case 3:
                        {
                            //作GBX
                            Gen.COM($"affected_layer,name=gbx,mode=single,affected=yes");
                            string gbx_lyr_text = @"$$JOB PAD $$DATE-MMDDYY";
                            Gen.COM("add_text" +
                                    ",attributes=no,type=string" +
                                    $",x={add_job_date_x},y={add_job_date_y}" +
                                    $",text={gbx_lyr_text},x_size={x_size},y_size={y_size}" +
                                    $",w_factor={w_factor}" +
                                    $",polarity={GenTools.POS_NEG["P"]}" +
                                    $",angle={text_angle},mirror={mirror_text},fontname=standard,ver=1");
                            Gen.COM($"affected_layer,name=gbx,mode=single,affected=no");

                            List<List<string>> via_tnt_list = new List<List<string>>() { layers.Via, layers.Tent };
                            for (var j = 0; j < via_tnt_list.Count; j++)
                            {
                                foreach (var lyr in via_tnt_list[j])
                                {
                                    Gen.COM($"affected_layer,name={lyr},mode=single,affected=yes");
                                    string lyr_text = @"$$JOB VIA $$DATE-MMDDYY";
                                    double text_len = lyr_text.Length; //廠料文字長度
                                    if (lyr == "v1" || lyr == "t1")
                                    {
                                        text_len = GenTools.DoubleRound(text_len * 0.09);
                                        add_job_date_y = min_y + 2.1;
                                        text_angle = 270;
                                        mirror_text = "no";
                                    }
                                    else
                                    {
                                        text_len = GenTools.DoubleRound(text_len * 0.09);
                                        add_job_date_y = min_y + 1.195 + text_len;
                                        text_angle = 90;
                                        mirror_text = "yes";
                                    }

                                    Gen.COM("add_text" +
                                            ",attributes=no,type=string" +
                                            $",x={add_job_date_x},y={add_job_date_y}" +
                                            $",text={lyr_text},x_size={x_size},y_size={y_size}" +
                                            $",w_factor={w_factor}" +
                                            $",polarity={GenTools.POS_NEG["P"]}" +
                                            $",angle={text_angle},mirror={mirror_text},fontname=standard,ver=1");

                                    Gen.COM($"affected_layer,name={lyr},mode=single,affected=no");
                                }
                            }
                        }
                        break;
                    //TODO: HDI板需調整
                    // 4: inner layer
                    case 4:
                        if (text_lalyer.Count > 0)
                        {
                            //var dataGridRow = mainWindow.laminationParametersDataGrid.Items[0] as DataGridRow;
                            int lamCount = mainWindow.laminationParametersDataGrid.Items.Count; //獲取內層數量
                            if (lamCount == 1)
                            {
                                string lyr_text = "";
                                int lyr_count = 1;
                                for (int j = 0; j < layers.InnerList.Count; j++)
                                {
                                    Gen.COM($"affected_layer,name={layers.InnerList[j]},mode=single,affected=yes");
                                    add_job_date_x = max_x + 0.25;
                                    add_job_date_y = min_y + 1.4;
                                    lyr_count++;
                                    string lyrNum = Regex.Match(layers.InnerList[j], @"\d+").Value;
                                    if (layers.InnerList[j][0] == 'e')
                                    {
                                        lyr_text = $"$$JOB L{lyrNum}E $$DATE-MMDDYY";
                                    }
                                    else
                                    {
                                        lyr_text = $"$$JOB L{lyrNum} $$DATE-MMDDYY";
                                    }

                                    if (lyr_count % 2 == 0)
                                    {
                                        text_angle = 270;
                                        mirror_text = "no"; //廠料文字鏡像
                                    }
                                    else
                                    {
                                        text_angle = 90;
                                        mirror_text = "yes"; //廠料文字鏡像
                                        double text_len = lyr_text.Length; //廠料文字長度
                                        text_len = GenTools.DoubleRound(text_len * 0.09);
                                        add_job_date_y = min_y + 1.195 + text_len + 0.09;
                                    }

                                    Gen.COM("add_pad" +
                                            $",attributes=no,x={pn_dc_clearance_box_x},y={pn_dc_clearance_box_y}" +
                                            $",symbol=rect189x2252,polarity=negative,angle=0,mirror=no" +
                                            $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
                                    Gen.COM("add_text" +
                                            ",attributes=no,type=string" +
                                            $",x={add_job_date_x},y={add_job_date_y}" +
                                            $",text={lyr_text},x_size={x_size},y_size={y_size}" +
                                            $",w_factor={w_factor}" +
                                            $",polarity={GenTools.POS_NEG["P"]}" +
                                            $",angle={text_angle},mirror={mirror_text},fontname=standard,ver=1");
                                    Gen.COM($"affected_layer,name={layers.InnerList[j]},mode=single,affected=no");
                                }
                            }
                        }

                        break;
                }
            }
        }

        /// <summary>
        /// 防焊CCD光學點PAD
        /// </summary>
        private static void Add_CCD_Fid_Pad()
        {
            List<int> sm_ccd_pad_ang = new List<int>() { 0, 180, 0, 180, 270, 270, 90, 90 };

            List<double> ccd_fid_pad_x = new List<double>()
            {
                // 左下H
                min_x + 1.7,
                // 右下H
                max_x - 1.7,
                // 左上H
                min_x + 1.7,
                // 右上H
                max_x - 0.65,
                // 左下V
                min_x - 0.2,
                // 右下V
                max_x + 0.2,
                // 右上V
                max_x + 0.2,
                //左上V
                min_x - 0.2,
            };
            List<double> ccd_fid_pad_y = new List<double>()
            {
                // 左下H
                min_y - 0.3,
                // 右下H
                min_y - 0.3,
                // 左上H
                max_y + 0.3,
                // 右上H
                max_y + 0.3,
                // 左下V
                min_y + 1.65,
                // 右下V
                min_y + 1.5,
                // 右上V
                max_y - 1.6,
                // 左上V
                max_y - 1.55,
            };
            List<List<string>> out_sm_ccd_lyr = new List<List<string>>()
            {
                layers.Outer, layers.SolderMask, layers.SilkScreen
            };
            for (int i = 0; i < out_sm_ccd_lyr.Count; i++)
            {
                string ccd_sym = "";
                if (i == 0)
                {
                    if (out_sm_ccd_lyr[i].Count > 0)
                    {
                        ccd_sym = sym.Pnl_Ccd_Out_Fid;
                    }
                    else
                    {
                        continue;
                    }
                }
                else if (i == 1)
                {
                    if (out_sm_ccd_lyr[i].Count > 0)
                    { 
                        ccd_sym = sym.Pnl_Ccd_Sm_Fid;
                    }else
                    {
                        continue;
                    }
                }
                else if (i == 2)
                {
                    if (out_sm_ccd_lyr[i].Count > 0)
                    {
                        ccd_sym = "r40";
                    }else
                    {
                        continue;
                    }
                }

                foreach (var workLyr in out_sm_ccd_lyr[i])
                {
                    Gen.COM($"affected_layer,name={workLyr},mode=single,affected=yes");
                }

                for (int j = 0; j < ccd_fid_pad_x.Count; j++)
                {
                    Gen.COM("add_pad" +
                            ",attributes=no" +
                            $",x={ccd_fid_pad_x[j]},y={ccd_fid_pad_y[j]}" +
                            $",symbol={ccd_sym}" +
                            $",polarity=positive,angle={sm_ccd_pad_ang[j]}" +
                            $",mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
                }

                foreach (var workLyr in out_sm_ccd_lyr[i])
                {
                    Gen.COM($"affected_layer,name={workLyr},mode=single,affected=no");
                }
            }
        }

        /// <summary>
        /// 增加C防焊CCD孔+pad
        /// </summary>
        private static void Add_SM_CCD()
        {
            if (layers.Outer.Count > 0)
            {
                foreach (string outer in layers.Outer)
                {
                    Gen.COM($"affected_layer,name={outer},mode=single,affected=yes");
                }

                //運算符重載
                Gen.COM($" add_pad" +
                        $",attributes=no" +
                        $",x={min_x - 0.2362},y={CenterY - 4.9213}" +
                        $",symbol=r144,polarity={GenTools.POS_NEG["N"]}" +
                        $",angle=0,mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
                Gen.COM($" add_pad" +
                        $",attributes=no" +
                        $",x={min_x - 0.2362},y={CenterY + 5.1181}" +
                        $",symbol=r144,polarity={GenTools.POS_NEG["N"]}" +
                        $",angle=0,mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
                GenTools.Clear_Layers();
            }

            string[] drlgbx = { "drl", "gbx" }; //程式一開始已經測試是否存在這兩個層別
            foreach (string dg in drlgbx)
            {
                Gen.COM($"affected_layer,name={dg},mode=single,affected=yes");
            }

            Gen.COM($" add_pad" +
                    $",attributes=no" +
                    $",x={min_x - 0.2362},y={CenterY - 4.9213}" +
                    $",symbol=r125,polarity={GenTools.POS_NEG["P"]}" +
                    $",angle=0,mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
            Gen.COM($" add_pad" +
                    $",attributes=no" +
                    $",x={min_x - 0.2362},y={centerY + 5.1181}" +
                    $",symbol=r125,polarity={GenTools.POS_NEG["P"]}" +
                    $",angle=0,mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
            GenTools.Clear_Layers();
        }

        /// <summary>
        /// 增加外層十字型PAD
        /// </summary>
        /// <returns></returns>
        private static void Add_Ten_Pad()
        {
            //外層十字型PAD
            List<double> ten_sym_pad_x = new List<double>()
            {
                GenTools.DoubleRound(Frame_x / 3),
                GenTools.DoubleRound(Frame_x / 3 * 2),
                GenTools.DoubleRound(Frame_x / 3),
                GenTools.DoubleRound(Frame_x / 3 * 2),
                min_x - 0.2,
                min_x - 0.2,
                GenTools.DoubleRound(max_x + 0.2),
                GenTools.DoubleRound(max_x + 0.2),
            };
            List<double> ten_sym_pad_y = new List<double>()
            {
                min_y - 0.2,
                min_y - 0.2,
                GenTools.DoubleRound(max_y + 0.2),
                GenTools.DoubleRound(max_y + 0.2),
                GenTools.DoubleRound(Frame_y / 3),
                GenTools.DoubleRound(Frame_y / 3 * 2),
                GenTools.DoubleRound(Frame_y / 3),
                GenTools.DoubleRound(Frame_y / 3 * 2 + 0.5),
            };

            List<List<string>> ccd_fid_layers =
                new List<List<string>>() { layers.Outer, layers.SolderMask, layers.SilkScreen };

            List<string> padList = new List<string>()
            {
                sym.Pnl_Ten_Pad,
                sym.Pnl_Ten_Pad_Sm,
                sym.Pnl_Ten_Pad_Ss
            };
            for (int i = 0; i < ccd_fid_layers.Count; i++)
            {
                if (ccd_fid_layers[i].Count > 0)
                {
                    foreach (var ccd_fid_lyr in ccd_fid_layers[i])
                    {
                        Gen.COM($"affected_layer,name={ccd_fid_lyr},mode=single,affected=yes");
                    }

                    for (int j = 0; j < ten_sym_pad_x.Count; j++)
                    {
                        Gen.COM("add_pad" +
                                ",attributes=no" +
                                $",x={ten_sym_pad_x[j]},y={ten_sym_pad_y[j]}" +
                                $",symbol={padList[i]}" +
                                $",polarity=positive" +
                                $",angle=0" +
                                $",mirror=no" +
                                $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1 ");
                    }

                    GenTools.Clear_Layers();
                }
            }
        }

        /// <summary>
        ///  增加板角L型標記
        /// </summary>
        private static void Add_L_Mark()
        {
            List<int> angle = new List<int>() { 0, 90, 180, 270 };

            List<double> l_mark_x = new List<double>()
            {
                -0.02,
                -0.02,
                Frame_x + 0.02,
                Frame_x + 0.02,
            };

            List<double> l_mark_y = new List<double>()
            {
                -0.02,
                Frame_y + 0.02,
                Frame_y + 0.02,
                -0.02,
            };

            List<double> l_mark_far_x = new List<double>()
            {
                -Frame_Dist_x,
                -Frame_Dist_x,
                Frame_x + Frame_Dist_x,
                Frame_x + Frame_Dist_x,
            };
            List<double> l_mark_far_y = new List<double>()
            {
                -Frame_Dist_y,
                Frame_y + Frame_Dist_y,
                Frame_y + Frame_Dist_y,
                -Frame_Dist_y,
            };

            if (layers.Outer.Count > 0)
            {
                //外層十字型PAD
                foreach (string outer in layers.Outer)
                {
                    Gen.COM($"affected_layer,name={outer},mode=single,affected=yes");
                }

                for (int i = 0; i < l_mark_x.Count; i++)
                {
                    Gen.COM("add_pad" +
                            ",attributes=no" +
                            $",x={l_mark_x[i]},y={l_mark_y[i]}" +
                            $",symbol={sym.Pnl_L_Mark}" +
                            $",polarity=positive" +
                            $",angle={angle[i]}" +
                            $",mirror=no" +
                            $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1 ");
                }

                string pos_neg_ = "";

                if (mainWindow.outter_pos.IsChecked == true)
                {
                    pos_neg_ = "negative";
                }
                else if (mainWindow.outter_neg.IsChecked == true)
                {
                    pos_neg_ = "positive";
                }

                for (int i = 0; i < l_mark_far_x.Count; i++)
                {
                    Gen.COM("add_pad" +
                            ",attributes=no" +
                            $",x={l_mark_far_x[i]},y={l_mark_far_y[i]}" +
                            $",symbol={sym.Pnl_L_Mark_N}" +
                            $",polarity={pos_neg_}" +
                            $",angle={angle[i]}" +
                            $",mirror=no" +
                            $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1 ");
                }

                GenTools.Clear_Layers();
            }

            //v1,t1也增加L Mark(避免在v1,t1底過度片繪製過小)
            foreach (var v in layers.Via)
            {
                Gen.COM($"affected_layer,name={v},mode=single,affected=yes");
            }

            foreach (var t in layers.Tent)
            {
                Gen.COM($"affected_layer,name={t},mode=single,affected=yes");
            }

            for (int i = 0; i < l_mark_far_x.Count; i++)
            {
                Gen.COM("add_pad" +
                        ",attributes=no" +
                        $",x={l_mark_far_x[i]},y={l_mark_far_y[i]}" +
                        $",symbol={sym.Pnl_L_Mark_N}" +
                        $",polarity=positive" +
                        $",angle={angle[i]}" +
                        $",mirror=no" +
                        $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1 ");
            }

            GenTools.Clear_Layers();
        }

        /// <summary>
        ///增加印刷PIN PAD
        /// </summary>
        /// <param name="print_pin">The print pin.</param>
        private static void Add_Pin_pad(Tooling print_pin, ENUM_PIN_LYR out_inner_smsstentvia)
        {
            if (out_inner_smsstentvia == ENUM_PIN_LYR.OUTTER)
            {
                if (layers.Outer.Count > 0)
                {
                    //增加外框印刷PIN PAD
                    foreach (string outer in layers.Outer)
                    {
                        Gen.COM($"affected_layer,name={outer},mode=single,affected=yes");

                        //如果有SignalEmpty層別，則增加SignalEmpty層別的印刷PIN PAD
                        if (layers.Inner.Count == 0 && layers.SignalEmpty.Count > 0)
                        {
                            foreach (var se in layers.SignalEmpty)
                            {
                                Gen.COM($"affected_layer,name={se},mode=single,affected=yes");
                            }
                        }

                        for (int i = 0; i < print_pin.PrintPinOuterX.Count; i++)
                        {
                            //CCD PAD
                            if (print_pin.Ccd_Pad_Loc.IndexOf(i) != -1)
                            {
                                string ccd_pad = "";
                                //外層正片
                                ccd_pad = mainWindow.outter_pos.IsChecked == true
                                    ? sym.Pnl_Pin_Ccd_Pad_P
                                    : sym.Pnl_Pin_Ccd_Pad_N;

                                Gen.COM("add_pad" +
                                        ",attributes=no" +
                                        $",x={print_pin.PrintPinOuterX[i]},y={print_pin.PrintPinOuterY[i]}" +
                                        $",symbol={ccd_pad}" +
                                        $",polarity=positive" +
                                        $",angle=0,mirror=no" +
                                        $",nx=1,ny=1,dx=0,dy=0" +
                                        $",xscale=1,yscale=1");
                            }
                            else
                            {
                                Gen.COM("add_pad" +
                                        ",attributes=no" +
                                        $",x={print_pin.PrintPinOuterX[i]},y={print_pin.PrintPinOuterY[i]}" +
                                        $",symbol={sym.Pnl_Out_Pin_Pad}" +
                                        $",polarity=positive" +
                                        $",angle=0,mirror=no" +
                                        $",nx=1,ny=1,dx=0,dy=0" +
                                        $",xscale=1,yscale=1");
                            }
                        }
                    }
                }

                Gen.COM($"affected_layer,mode=all,affected=no");
            }
            else if (out_inner_smsstentvia == ENUM_PIN_LYR.SMSSTENTVIA ||
                     out_inner_smsstentvia == ENUM_PIN_LYR.DRLINNGBX)
            {
                string sym_ = "";
                if (out_inner_smsstentvia == ENUM_PIN_LYR.SMSSTENTVIA)
                {
                    List<List<string>> s_list = new List<List<string>>()
                    {
                        layers.SmssList, layers.Via, layers.Tent,
                    };

                    foreach (var list in s_list)
                    {
                        if (list.Count > 0)
                        {
                            foreach (var lyr_ in list)
                            {
                                Gen.COM($"affected_layer,name={lyr_},mode=single,affected=yes");
                            }
                        }
                    }

                    sym_ = "donut_r180x150";
                }
                else if (out_inner_smsstentvia == ENUM_PIN_LYR.DRLINNGBX)
                {
                    string[] drlgbx = { "drl", "gbx" };
                    foreach (string dg in drlgbx)
                    {
                        Gen.COM($"affected_layer,name={dg},mode=single,affected=yes");
                    }

                    sym_ = "r125";
                }

                for (int i = 0; i < print_pin.PrintPinOuterX.Count; i++)
                {
                    Gen.COM("add_pad" +
                            ",attributes=no" +
                            $",x={print_pin.PrintPinOuterX[i]},y={print_pin.PrintPinOuterY[i]}" +
                            $",symbol={sym_}" +
                            $",polarity=positive" +
                            $",angle=0,mirror=no" +
                            $",nx=1,ny=1,dx=0,dy=0" +
                            $",xscale=1,yscale=1");
                }
                Gen.COM($"affected_layer,mode=all,affected=no");
            }
            else if (out_inner_smsstentvia == ENUM_PIN_LYR.INNER)
            {
                if (layers.Inner.Count > 0)
                {
                    foreach (string inner in layers.InnerList)
                    {
                        Gen.COM($"affected_layer,name={inner},mode=single,affected=yes");
                    }

                    List<string> in_pad_sym = new List<string>()
                    {
                        sym.Pnl_Inn_Pin_Pad_N,
                        sym.Pnl_Inn_Pin_Pad_P,
                    };
                    for (int s = 0; s < in_pad_sym.Count; s++)
                    {
                        for (int i = 0; i < print_pin.PrintPinInnerX.Count; i++)
                        {
                            Gen.COM("add_pad" +
                                    ",attributes=no" +
                                    $",x={print_pin.PrintPinInnerX[i]},y={print_pin.PrintPinInnerY[i]}" +
                                    $",symbol={in_pad_sym[s]}" +
                                    $",polarity=positive" +
                                    $",angle=0,mirror=no" +
                                    $",nx=1,ny=1,dx=0,dy=0" +
                                    $",xscale=1,yscale=1");
                        }
                    }
                    Gen.COM($"affected_layer,mode=all,affected=no");
                    ////增加內層INN孔
                    Gen.COM($"affected_layer,name=inn,mode=single,affected=yes");
                    for (int i = 0; i < print_pin.PrintPinInnerX.Count; i++)
                    {
                        Gen.COM("add_pad" +
                                ",attributes=no" +
                                $",x={print_pin.PrintPinInnerX[i]},y={print_pin.PrintPinInnerY[i]}" +
                                $",symbol=r125" +
                                $",polarity=positive" +
                                $",angle=0,mirror=no" +
                                $",nx=1,ny=1,dx=0,dy=0" +
                                $",xscale=1,yscale=1");
                    }
                    Gen.COM($"affected_layer,name=inn,mode=single,affected=no");
                }
            }

            Gen.COM($"affected_layer,mode=all,affected=no");
        }

        /// <summary>
        /// 內、外框鋪銅
        /// </summary>
        private static void Do_Patttern_Fill()
        {
            bool isMultiLyr = false;
            bool isNegOuter = false;
            if (layers.Inner.Count > 0)
            {
                isMultiLyr = true;
                Layer_Pattern_Fill(layers.InnerList, 0, 0,
                    100, 100,
                    0.1, 0.1,
                    "s160",
                    0, 0);

                int isOdd = 0;
                foreach (var layer in layers.InnerList)
                {
                    isOdd++;
                    Gen.COM($"affected_layer,name={layer},mode=single,affected=yes");
                    string sym_ = "";
                    if (isOdd % 2 == 0)
                    {
                        sym_ = $"{sym.Pnl_Ccd_Inner_Frame_1}";
                    }
                    else
                    {
                        sym_ = $"{sym.Pnl_Ccd_Inner_Frame_2}";
                    }

                    Gen.COM("add_pad" +
                            $",attributes=no,x={CenterX},y={CenterY}" +
                            $",symbol={sym_}" +
                            $",polarity={GenTools.POS_NEG["P"]},angle=0" +
                            ",mirror=no,nx=1,ny=1,dx=0,dy=0" +
                            ",xscale=1,yscale=1");
                    Gen.COM($"affected_layer,name={layer},mode=single,affected=no");
                }
            }

            if (layers.Outer.Count > 0)
            {
                string sym_ = "";

                if (mainWindow.outter_pos.IsChecked == true) //正片流程
                {
                    Layer_Pattern_Fill(layers.Outer,
                        -.1, -.1,
                        100, 100,
                        0.1, 0.1,
                        "0");
                    if (layers.Inner.Count == 0 && layers.SignalEmpty.Count > 0)
                    {
                        Layer_Pattern_Fill(layers.SignalEmpty,
                            -.1, -.1,
                            100, 100,
                            0.1, 0.1,
                            "0");
                    }

                    sym_ = $"{sym.Pnl_Ccd_Frame_P}";
                }
                else if (mainWindow.outter_neg.IsChecked == true) //負片流程
                {
                    Layer_Pattern_Fill(layers.Outer, 0, 0,
                        100, 100,
                        0.1, 0.1,
                        "s200",
                        0, 0);
                    if (layers.Inner.Count == 0 && layers.SignalEmpty.Count > 0)
                    {
                        Layer_Pattern_Fill(layers.Outer, 0, 0,
                            100, 100,
                            0.1, 0.1,
                            "s200",
                            0, 0);
                    }

                    sym_ = $"{sym.Pnl_Ccd_Frame_N}";

                    isNegOuter = true;
                }

                foreach (var layer in layers.Outer)
                {
                    Gen.COM($"affected_layer,name={layer},mode=single,affected=yes");
                }

                Gen.COM("add_pad" +
                        $",attributes=no,x={CenterX},y={CenterY}" +
                        $",symbol={sym_}" +
                        $",polarity={GenTools.POS_NEG["P"]},angle=0" +
                        ",mirror=no,nx=1,ny=1,dx=0,dy=0" +
                        ",xscale=1,yscale=1");

                foreach (var layer in layers.Outer)
                {
                    Gen.COM($"affected_layer,name={layer},mode=single,affected=no");
                }
            }

            if (isMultiLyr || isNegOuter)
            {
                //CCD板框填滿銅
                string ccd_fill = "ccd_coooper_fill";
                if (GenTools.Layer_IsExist(Job, workName, ccd_fill))
                    Gen.COM($"delete_layer,layer={ccd_fill}");
                GenTools.Layer_Create(ccd_fill, "misc", GenTools.layerType["sig"]);
                Gen.COM($"affected_layer,name={ccd_fill},mode=single,affected=yes");

                double x_dis = 10.96;
                double y_dis = 12.457;
                //x:centerX->10.96 centerY->12.457
                Gen.COM("add_surf_strt,surf_type=feature");
                Gen.COM($"add_surf_poly_strt,x={CenterX - x_dis},y={CenterY - y_dis}");
                Gen.COM($"add_surf_poly_seg,x={CenterX - x_dis},y={CenterY + y_dis}");
                Gen.COM($"add_surf_poly_seg,x={CenterX + x_dis},y={CenterY + y_dis}");
                Gen.COM($"add_surf_poly_seg,x={CenterX + x_dis},y={CenterY - y_dis}");
                Gen.COM($"add_surf_poly_seg,x={CenterX - x_dis},y={CenterY - y_dis}");
                Gen.COM("add_surf_poly_end");
                Gen.COM($"add_surf_end,attributes=no,polarity={GenTools.POS_NEG["P"]}");

                Gen.COM("add_surf_strt,surf_type=feature");
                Gen.COM($"add_surf_poly_strt,x={-frame_dist_x - 0.2},y={-frame_dist_y - 0.2}");
                Gen.COM($"add_surf_poly_seg,x={-frame_dist_x - 0.2},y={frame_y + frame_dist_y + 0.2}");
                Gen.COM($"add_surf_poly_seg,x={frame_x + frame_dist_x + 0.2},y={frame_y + frame_dist_y + 0.2}");
                Gen.COM($"add_surf_poly_seg,x={frame_x + frame_dist_x + 0.2},y={-frame_dist_y - 0.2}");
                Gen.COM($"add_surf_poly_seg,x={-frame_dist_x - 0.2},y={-frame_dist_y - 0.2}");
                Gen.COM("add_surf_poly_end");
                Gen.COM($"add_surf_end,attributes=no,polarity={GenTools.POS_NEG["N"]}");
                //使用PAD方式
                /*
                         Gen.COM("add_pad" +
                $",attributes=no,x={centerX},y={centerY},symbol=rect{x_dis+x_dis}x{y_dis+y_dis}" +
                $",polarity=positive,angle=0,mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
        double x_dis_n = frame_x + frame_dist_x + 0.2 + Frame_Dist_x + 0.2;
        double y_dis_n = frame_y + frame_dist_y + 0.2 + Frame_Dist_y + 0.2;
        Gen.COM("add_pad" +
                $",attributes=no,x={centerX},y={centerY},symbol=rect{x_dis_n}x{y_dis_n}" +
                $",polarity=positive,angle=0,mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
                 */

                Gen.COM("sel_contourize" +
                        ",accuracy=0.25,break_to_islands=yes" +
                        ",clean_hole_size=3,clean_hole_mode=x_and_y");
                if (isMultiLyr)
                {
                    Gen.COM($"affected_layer,name={ccd_fill},mode=single,affected=yes");
                    foreach (string layer in layers.InnerList)
                    {
                        Gen.COM("sel_copy_other" +
                                $",dest=layer_name,target_layer={layer}" +
                                $",invert=no,dx=0,dy=0,size=0" +
                                $",x_anchor=0,y_anchor=0,rotation=0,mirror=none");
                    }

                    Gen.COM($"affected_layer,name={ccd_fill},mode=single,affected=no");
                }

                if (isNegOuter)
                {
                    Gen.COM($"affected_layer,name={ccd_fill},mode=single,affected=yes");
                    foreach (string layer in layers.Outer)
                    {
                        Gen.COM("sel_copy_other" +
                                $",dest=layer_name,target_layer={layer}" +
                                $",invert=no,dx=0,dy=0,size=0" +
                                $",x_anchor=0,y_anchor=0,rotation=0,mirror=none");
                    }

                    Gen.COM($"affected_layer,name={ccd_fill},mode=single,affected=no");
                }

                Gen.COM($"delete_layer,layer={ccd_fill}");
            }

            //增加內層導流條
            if (isMultiLyr)
            {
                Add_Inner_Strip();
            }
        }

        /// <summary>
        /// 折斷邊鋪銅Layers the pattern fill.
        /// </summary>
        /// <param name="layers">The layers.</param>
        /// <param name="step_mg_x">The step mg x.</param>
        /// <param name="step_mg_y">The step mg y.</param>
        /// <param name="step_max_dist_x">The step maximum dist x.</param>
        /// <param name="step_max_dist_y">The step maximum dist y.</param>
        /// <param name="sr_mg_x">The sr mg x.</param>
        /// <param name="sr_mg_y">The sr mg y.</param>
        /// <param name="trim_line">The trim line.</param>
        /// <param name="sr_max_dist_x">The sr maximum dist x.</param>
        /// <param name="sr_max_dist_y">The sr maximum dist y.</param>
        public static void Layer_Pattern_Fill(
            List<string> layers,
            double step_mg_x, double step_mg_y, //下料邊往的銅
            double step_max_dist_x, double step_max_dist_y, //下料邊往內鋪銅最大距離
            double sr_mg_x, double sr_mg_y, //sr往外的的內縮銅箔距離
            string trim_line,
            double sr_max_dist_x = 0f, double sr_max_dist_y = 0f
        ) //限制sr margin往外鋪銅最大距離(0=不限制)
        {
            if (layers.Count > 0)
            {
                foreach (var layer in layers)
                {
                    Gen.COM($"affected_layer,name={layer},mode=single,affected=yes");
                }

                Gen.COM($"sr_fill,polarity=positive," +
                        $"step_margin_x={step_mg_x}" +
                        $",step_margin_y={step_mg_y}" +
                        $",step_max_dist_x={step_max_dist_x}" +
                        $",step_max_dist_y={step_max_dist_y}" +
                        $",sr_margin_x={sr_mg_x}" +
                        $",sr_margin_y={sr_mg_y}" +
                        $",sr_max_dist_x={sr_max_dist_x}" +
                        $",sr_max_dist_y={sr_max_dist_y}" +
                        ",nest_sr=no,consider_feat=no,consider_drill=no,consider_rout=no" +
                        ",dest=affected_layers,attributes=no");
                if (step_mg_x == 0 && trim_line != "0")
                {
                    Gen.COM("add_polyline_strt");
                    Gen.COM($"add_polyline_xy, x ={-Frame_Dist_x}, y ={-Frame_Dist_y} ");
                    Gen.COM($"add_polyline_xy, x ={-Frame_Dist_x}, y ={Frame_y + Frame_Dist_y} ");
                    Gen.COM($"add_polyline_xy, x ={Frame_x + Frame_Dist_x}, y ={Frame_y + Frame_Dist_y} ");
                    Gen.COM($"add_polyline_xy, x ={Frame_x + Frame_Dist_x}, y ={-Frame_Dist_y} ");
                    Gen.COM($"add_polyline_xy, x ={-Frame_Dist_x}, y ={-Frame_Dist_y} ");
                    Gen.COM($"add_polyline_end, attributes = no, symbol ={trim_line}, polarity = negative");

                    Gen.COM("sel_contourize" +
                            ",accuracy=0.25,break_to_islands=yes" +
                            ",clean_hole_size=3,clean_hole_mode=x_and_y");
                }

                GenTools.Clear_Layers();
            }
        }

        public static void AutoPanel()
        {
            string selStep = mainWindow.stpSlect.Text;

            //check datum is offset zero point
            Gen.INFO($"-t step -e {Job}/{selStep} -m script -d PROF_LIMITS");
            double prof_limit_xmin = GenTools.DoubleRound(Math.Abs(double.Parse(Gen.GetInfo("gPROF_LIMITSxmin")[0])));
            double prof_limit_xmax = GenTools.DoubleRound(Math.Abs(double.Parse(Gen.GetInfo("gPROF_LIMITSxmax")[0])));
            double prof_limit_ymin = GenTools.DoubleRound(Math.Abs(double.Parse(Gen.GetInfo("gPROF_LIMITSymin")[0])));
            double prof_limit_ymax = GenTools.DoubleRound(Math.Abs(double.Parse(Gen.GetInfo("gPROF_LIMITSymax")[0])));

            //absolute limits

            string auto_step = mainWindow.stpSlect.Text;
            string auto_angle = mainWindow.shipRotate.Text;
            if (auto_angle == "0")
            {
                Gen.COM($"sr_tab_add,line=0,step={auto_step},x={prof_limit_xmin},y={prof_limit_ymin}," +
                        $"nx={Xstep_num},ny={ystep_num},dx={Six + xstep_spec},dy={Siy + ystep_spec}");
            }
            else
            {
                Gen.COM($"sr_tab_add,line=0,step={auto_step},x={prof_limit_ymin} ,y= {prof_limit_xmax}," +
                        $"nx={Xstep_num},ny={ystep_num},dx={Six + xstep_spec},dy={Siy + ystep_spec},angle={auto_angle}");
            }
        }

        /// <summary>
        /// 增加流膠條 Adds the inner strip.
        /// </summary>
        public static void Add_Inner_Strip()
        {
            //創建tmp層

            //strip rule-4x each long side ,2x each short side

            double add_inner_copper_x1 = 0;
            double add_inner_copper_x12 = -frame_dist_x;

            double add_inner_copper_y1 = Min_y + 6;
            double add_inner_copper_y12 = add_inner_copper_y1 - 0.5;
            double add_inner_copper_y13 = add_inner_copper_y1 - 0.5 + 0.24;
            double add_inner_copper_y14 = add_inner_copper_y1 + 0.24;

            double add_inner_copper_y2 = Min_y + 3.5;
            double add_inner_copper_y22 = add_inner_copper_y2 - 0.5;
            double add_inner_copper_y23 = add_inner_copper_y2 - 0.5 + 0.24;
            double add_inner_copper_y24 = add_inner_copper_y2 + 0.24;

            double add_inner_copper_y3 = Max_y - 6;
            double add_inner_copper_y32 = add_inner_copper_y3 - 0.5;
            double add_inner_copper_y33 = add_inner_copper_y3 - 0.5 + 0.24;
            double add_inner_copper_y34 = add_inner_copper_y3 + 0.24;

            double add_inner_copper_y4 = Max_y - 3.5;
            double add_inner_copper_y42 = add_inner_copper_y4 - 0.5;
            double add_inner_copper_y43 = add_inner_copper_y4 - 0.5 + 0.24;
            double add_inner_copper_y44 = add_inner_copper_y4 + 0.24;

            List<string> ccd_str_lyr = new List<string>() { "ccd_strip_tmp_1", "ccd_strip_tmp_2" };
            foreach (string ccd_s_lyr in ccd_str_lyr)
            {
                if (GenTools.Layer_IsExist(Job, workName, ccd_s_lyr))
                    Gen.COM($"delete_layer,layer={ccd_s_lyr}");
                GenTools.Layer_Create(ccd_s_lyr, "misc", GenTools.layerType["sig"]);
            }

            string tmp_01 = "t_m_01";

            if (GenTools.Layer_IsExist(Job, workName, tmp_01))
                Gen.COM($"delete_layer,layer={tmp_01}");
            GenTools.Layer_Create(tmp_01, "misc", GenTools.layerType["sig"]);

            Gen.COM($"affected_layer,name={tmp_01},mode=single,affected=yes");

            //左邊第一顆
            Gen.COM("add_surf_strt,surf_type=feature");
            Gen.COM($"add_surf_poly_strt,x={add_inner_copper_x1},y={add_inner_copper_y1}");
            Gen.COM($"add_surf_poly_seg,x={add_inner_copper_x12},y={add_inner_copper_y12}");
            Gen.COM($"add_surf_poly_seg,x={add_inner_copper_x12},y={add_inner_copper_y13}");
            Gen.COM($"add_surf_poly_seg,x={add_inner_copper_x1},y={add_inner_copper_y14}");
            Gen.COM($"add_surf_poly_seg,x={add_inner_copper_x1},y={add_inner_copper_y1}");
            Gen.COM("add_surf_poly_end");
            Gen.COM("add_surf_end,attributes = no,polarity = positive");

            //左邊第二顆
            Gen.COM($"add_surf_strt, surf_type = feature");
            Gen.COM($"add_surf_poly_strt, x = {add_inner_copper_x12},y = {add_inner_copper_y2}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x1},y = {add_inner_copper_y22}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x1},y = {add_inner_copper_y23}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x12},y = {add_inner_copper_y24}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x12},y = {add_inner_copper_y2}");
            Gen.COM("add_surf_poly_end");
            Gen.COM("add_surf_end, attributes = no, polarity = positive");
            //左邊第三顆
            Gen.COM($"add_surf_strt, surf_type = feature");
            Gen.COM($"add_surf_poly_strt, x = {add_inner_copper_x1},y = {add_inner_copper_y3}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x12},y = {add_inner_copper_y32}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x12},y = {add_inner_copper_y33}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x1},y = {add_inner_copper_y34}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x1},y = {add_inner_copper_y3}");
            Gen.COM("add_surf_poly_end");
            Gen.COM("add_surf_end, attributes = no, polarity = positive");

            //左邊第四顆

            Gen.COM($"add_surf_strt, surf_type = feature");
            Gen.COM($"add_surf_poly_strt, x = {add_inner_copper_x12},y = {add_inner_copper_y4}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x1},y = {add_inner_copper_y42}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x1},y = {add_inner_copper_y43}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x12},y = {add_inner_copper_y44}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x12},y = {add_inner_copper_y4}");
            Gen.COM("add_surf_poly_end");
            Gen.COM("add_surf_end, attributes = no, polarity = positive");

            Gen.COM($"sel_transform,mode=anchor" +
                    $",oper=mirror" +
                    $",duplicate=yes" +
                    $",x_anchor={centerX},y_anchor=0" +
                    $",angle=0,x_scale=1,y_scale=1" +
                    $",x_offset=0,y_offset=0");
            Gen.COM($"sel_copy_other" +
                    $",dest=layer_name,target_layer={ccd_str_lyr[0]}" +
                    $",invert=yes,dx=0,dy=0" +
                    $",size=0,x_anchor=0" +
                    $",y_anchor=0,mirror=none,rotation=0");
            Gen.COM($"sel_transform" +
                    $",mode=axis,oper=y_mirror,duplicate=no" +
                    $",x_anchor=-0.3927834646,y_anchor=11.1881" +
                    $",angle=180,x_scale=1,y_scale=1,x_offset=0,y_offset=0");
            Gen.COM($"sel_move_other" +
                    $",target_layer={ccd_str_lyr[1]}" +
                    $",invert=yes" +
                    $",dx=0,dy=0" +
                    $",size=0,x_anchor=0,y_anchor=0,rotation=0,mirror=none");
            GenTools.Clear_Layers();

            Gen.COM($"affected_layer,name={tmp_01},mode=single,affected=yes");

            double add_inner_copper_y5 = 0;
            double add_inner_copper_y52 = -frame_dist_y;

            double add_inner_copper_x5 = Min_x + 2.6;
            double add_inner_copper_x52 = add_inner_copper_x5 + 0.5;
            double add_inner_copper_x53 = add_inner_copper_x5 + 0.5 + 0.24;
            double add_inner_copper_x54 = add_inner_copper_x5 + 0.24;

            double add_inner_copper_x6 = Max_x - 2.6;
            double add_inner_copper_x62 = add_inner_copper_x6 - 0.5;
            double add_inner_copper_x63 = add_inner_copper_x6 - 0.5 - 0.24;
            double add_inner_copper_x64 = add_inner_copper_x6 - 0.24;

            //下方第一顆

            Gen.COM($"add_surf_strt, surf_type = feature");
            Gen.COM($"add_surf_poly_strt, x = {add_inner_copper_x5},y = {add_inner_copper_y5}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x52},y = {add_inner_copper_y52}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x53},y = {add_inner_copper_y52}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x54},y = {add_inner_copper_y5}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x5},y = {add_inner_copper_y5}");
            Gen.COM("add_surf_poly_end");
            Gen.COM("add_surf_end, attributes = no, polarity = positive");

            //下方第二顆

            Gen.COM($"add_surf_strt, surf_type = feature");
            Gen.COM($"add_surf_poly_strt, x = {add_inner_copper_x6},y = {add_inner_copper_y5}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x62},y = {add_inner_copper_y52}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x63},y = {add_inner_copper_y52}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x64},y = {add_inner_copper_y5}");
            Gen.COM($"add_surf_poly_seg, x = {add_inner_copper_x6},y = {add_inner_copper_y5}");
            Gen.COM("add_surf_poly_end");
            Gen.COM("add_surf_end, attributes = no, polarity = positive");

            Gen.COM($"sel_transform" +
                    $",mode=anchor,oper=y_mirror" +
                    $",duplicate=yes" +
                    $",x_anchor=0,y_anchor={centerY}" +
                    $",angle=0" +
                    $",x_scale=1,y_scale=1" +
                    $",x_offset=0,y_offset=0");
            Gen.COM($"sel_copy_other" +
                    $",dest=layer_name,target_layer={ccd_str_lyr[0]}" +
                    $",invert=yes" +
                    $",dx=0,dy=0" +
                    $",size=0,x_anchor=0,y_anchor=0" +
                    $",mirror=none,rotation=0");
            Gen.COM($"sel_transform" +
                    $",mode=axis,oper=y_mirror" +
                    $",duplicate=no" +
                    $",x_anchor=-0.3927834646,y_anchor=11.1881" +
                    $",angle=180" +
                    $",x_scale=1,y_scale=1" +
                    $",x_offset=0,y_offset=0");
            Gen.COM($"sel_move_other" +
                    $",target_layer={ccd_str_lyr[1]}" +
                    $",invert=yes" +
                    $",dx=0,dy=0,size=0" +
                    $",x_anchor=0,y_anchor=0" +
                    $",rotation=0,mirror=none");

            Gen.COM($"delete_layer,layer={tmp_01}");
            GenTools.Clear_Layers();

            //複製內層導流條到 iner layer
            int isOdd = 0;
            foreach (string inn_lyr in layers.Inner)
            {
                Gen.COM("affected_layer,name=,mode=all,affected=no");
                isOdd++;
                if (isOdd % 2 == 1)
                {
                    Gen.COM($"affected_layer,name={ccd_str_lyr[0]},mode=single,affected=yes");
                }
                else
                {
                    Gen.COM($"affected_layer,name={ccd_str_lyr[1]},mode=single,affected=yes");
                }

                Gen.COM($"sel_copy_other" +
                        $",dest=layer_name,target_layer={inn_lyr}" +
                        $",invert=no" +
                        $",dx=0,dy=0" +
                        $",size=0" +
                        $",x_anchor=0,y_anchor=0" +
                        $",mirror=none,rotation=0");
            }

            Gen.COM($"delete_layer,layer={ccd_str_lyr[0]}");
            Gen.COM($"delete_layer,layer={ccd_str_lyr[1]}");
            ccd_str_lyr = null;
        }
    }
}