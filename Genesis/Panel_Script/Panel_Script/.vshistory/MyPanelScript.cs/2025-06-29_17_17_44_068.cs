    ...
    public static class MyPanelScript
    {
        ...
        public static void RunPanelScript()
        {
            ...
            if (!GenTools.Layer_IsExist(Job, workName, "via"))
                GenTools.Layer_Create("via", "board", GenTools.layerType["drill"]);
            if (!GenTools.Layer_IsExist(Job, workName, "t1"))
                GenTools.Layer_Create("t1", "board", GenTools.layerType["drill"]);
            ...
            {
                string temp_layer = "fill_copper_"; // Temporary layer name
                if (GenTools.Layer_IsExist(Job, Step, temp_layer))
                {
                    Gen.COM($"delete_layer,layer={temp_layer}");
                    GenTools.Layer_Create(temp_layer, "misc", GenTools.layerType["sig"]);
                }

                Gen.COM($"affected_layer,layer={temp_layer},mode=single,affected=yes");

                GenTools.Layer_Pattern_Fill(temp_layer, 0, 0,
                    100, 100,
                    0.1, 0.1,
                    frame_dist_x - 0.1, frame_dist_y - 0.1);
                Point p = new Point(x_min: 0, y_min: 0, x_max: frame_x, y_max: frame_y);
                GenTools.Add_Rec_Line(p);
