...
namespace Panel_Script
{
    public static class MyPanelScript
    {
        ...
        {
            double add_symbol_donut_x1 = GenTools.DoubleRound(min_x - (Wix - Frame_x) / 4);
            double add_symbol_donut_x2 = GenTools.DoubleRound(max_x + (Wix - Frame_x) / 4);
            double add_symbol_donut_y1 = GenTools.DoubleRound(min_y - (Wiy - Frame_y) / 2 + 0.3);
            double add_symbol_donut_y2 = GenTools.DoubleRound(max_y + (Wiy - Frame_y) / 2 - 0.3);
            ...
            foreach (string inner in layers.Inner)
            {
                ...
                symbol_dounut_list.Add(
                    string.Format("%dx%d", symbol_donut_width, symbol_donut_height));
            }

            symbol_donut_width += 5;
            string symbol_outer = string.Format("r%d", symbol_donut_width);
            symbol_donut_width += 15;
            string symbol_biggest = string.Format("r%d", symbol_donut_width);
            ...
            for (int i = 0; i < layers.Inner.Count; i++)
            {
                ...
                for (int j = 0; j < sym_dount_x.Count; j++)
                {
                    Gen.COM(("add_pad" +
                             ...
                             $",symbol={symbol_biggest},polarity=negative" +
                             $",angle=0,mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1"));
                    Gen.COM(("add_pad" +
                             $",attributes=no,x={sym_dount_x[j]},y={sym_dount_y[j]}" +
                             $",symbol=r{symbol_dounut_list[i]},polarity=positive" +
                             $",angle=0,mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1"));
                }
            ...
            for (int i = 0; i < sym_dount_x.Count; i++)
            {
                    Gen.COM(("add_pad" +
                             ...
                             $",symbol={symbol_outer},polarity=negative" +
                             $",angle=0,mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1"));
            }
            GenTools.Clear_Layers();
        }
