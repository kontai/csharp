...
namespace Panel_Script
{
    public static class MyPanelScript
    {
        ...
        {
            double tag_pad_x1 = min_x - 0.35;
            double tag_pad_x2 = max_x + 0.35;
            double tag_pad_x3 = (max_x - min_x) / 2 + 2.15;
            double tag_pad_x4 = (max_x - min_x) / 2 - 1.85;
            double tag_pad_y1 = (max_y - min_y) / 2;
            double tag_pad_y2 = max_y + (Wiy - Frame_x) / 2 - 1.5;
            double tag_pad_y3 = min_y - (Wiy - Frame_x) / 2 + 1.5;
            double tag_pad_y4 = (max_y - min_y) / 2 - 0.5;
            double tag_pad_y5 = min_y - (Wiy - Frame_x) / 2 + 1.5;
            double tag_pad_y6 = max_y + 0.35;
            double tag_pad_y7 = min_y - 0.35;
            List<double> tag_pad_list_x = new List<double>()
            {
                tag_pad_x1, tag_pad_x1,
                tag_pad_x1, tag_pad_x2,
                tag_pad_x2, tag_pad_x2,
                tag_pad_x3, tag_pad_x3,
                tag_pad_x4, tag_pad_x4,
            };
            List<double> tag_pad_list_y = new List<double>()
            {
                tag_pad_y1, tag_pad_y2,
                tag_pad_y3, tag_pad_y4,
                tag_pad_y2, tag_pad_y5,
                tag_pad_y6, tag_pad_y7,
                tag_pad_y6, tag_pad_y7,
            };
            List<int> tag_dr_ang_list = new List<int>()
            {
                0, 0,
                0, 180,
                180, 180,
            ...
            string sym_ = ""
            for (int i = 0; i < tag_pad_list_x.Count; i++)
            {
                Gen.COM("add_pad" +
                        $",attributes=no,x={tag_pad_list_x[i]},y={tag_pad_list_y[i]}" +
                        $",symbol={sym.Pnl_Inn_Tag},polarity=positive,angle=0,mirror=no" +
                        $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1"):;
                if (i == 0 || i == 3)
                {
                    sym_ = $"{sym.Pnl_Inn_Tag_Dr}";
                }
                else
                {
                    sym_ = $"{sym.Pnl_Inn_Tag_Dr2}";
                }

                Gen.COM("add_pad" +
                        $",attributes=no,x={tag_pad_list_x[i]},y={tag_pad_list_y[i]}" +
                        $",symbol={sym_},polarity=negative,angle={tag_dr_ang_list[i]}" +
...
COM add_pad,attributes=no,x=13.801538189,y=10.1156823819,symbol=lay-2
        ...
        private static void Add_CCD_Fid_Pad()
        {
            ...
            CCD_Fid_Pad ccd_fid_pad = new CCD_Fid_Pad(min_x, min_y, max_x, max_y);
            ...
            for (int i = 0; i < out_sm_ccd_lyr.Count; i++)
            {
                ...
                for (int j = 0; j < ccd_fid_pad.X.Count; j++)
                {
                    Gen.COM("add_pad" +
                            ",attributes=no" +
                            $",x={ccd_fid_pad.X[j]},y={ccd_fid_pad.Y[j]}" +
                            $",symbol={ccd_sym}" +
