...
namespace Panel_Script
{
    public static class MyPanelScript
    {
        ...
        public static void AutoPanel()
        {
            ...
            Gen.INFO($"-t step -e {Job}/{selStep} -m script -d LIMITS");
            double qLIMITxmin= DoubleRound(double.Parse(Gen.GetInfo("gLIMITSxmin")[0]));
            double qLIMITymin = DoubleRound(double.Parse(Gen.GetInfo("gLIMITSymin")[0]));

            string auto_step = mainWindow.stpSlect.Text;
            string auto_angle = mainWindow.shipRotate.Text;
            if (auto_angle == "0")
            {
                Gen.COM($"sr_tab_add,line=0,step={auto_step},x={Frame_Dist_x-qLIMITxmin},y={Frame_Dist_y-qLIMITymin}," +
                        $"nx={Xstep_num},ny={ystep_num},dx={Six + xstep_spec},dy={Siy + ystep_spec}");
            }
            else
            {
                Gen.COM($"sr_tab_add,line=0,step={auto_step},x={Frame_Dist_x-qLIMITxmin},y={Frame_Dist_y + Siy-qLIMITxmin}," +
