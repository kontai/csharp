        ...
        private static void Add_Special_Symbols()
        {
            ...
            if (layers.SolderMask.Count > 0)
            {
                foreach (string sm in layers.SolderMask)
                {
                    ...
                    double dam_text_y = min_y + 4.58;
                    string min_dam_size = mainWindow.min_dam_size.Text;
                    if (min_dam_size == null || min_dam_size == "") min_dam_size = "4";
                    double sym_size = 0.37;
                    double dam_text_size = GenTools.DoubleRound(double.Parse(min_dam_size) * 0.1);
                    if (layers.Full_Layer_Side[sm] == "top")
                    {
                        Gen.COM("add_pad" +
                                ",attributes=no" +
                                $",x={dam_text_x},y={dam_text_y},symbol={sym.Pnl_Tenting_Process}" +
                                $",polarity=positive,angle=0,mirror=no,nx=1,ny=1,dx=0,dy=0" +
                                $",xscale=1,yscale=1");
                        double no_mir_text = GenTools.DoubleRound(dam_text_y - sym_size / 2 - dam_text_size - 0.1);
                        Gen.COM("add_text" +
                                $",attributes=no,type=string,x={dam_text_x},y={no_mir_text}" +
                                $",text={min_dam_size},x_size=0.1,y_size=0.08,w_factor=0.833333333333333" +
                                $",polarity=positive,angle=270,mirror=no,fontname=chineset.shx,ver=1");
                    }
                    else if (layers.Full_Layer_Side[sm] == "bottom")
                    {
                        Gen.COM("add_pad" +
                                ",attributes=no" +
                                $",x={dam_text_x},y={dam_text_y},symbol={sym.Pnl_Tenting_Process}" +
                                $",polarity=positive,angle=90,mirror=yes,nx=1,ny=1,dx=0,dy=0" +
                                $",xscale=1,yscale=1");
                        double no_mir_text = GenTools.DoubleRound(dam_text_y + sym_size / 2 + dam_text_size + 0.1);
                        Gen.COM("add_text" +
                                $",attributes=no,type=string,x={dam_text_x},y={no_mir_text}" +
                                $",text={min_dam_size},x_size=0.1,y_size=0.08,w_factor=0.833333333333333" +
                                $",polarity=positive,angle=90,mirror=yes,fontname=chineset.shx,ver=1");
                    }
                }
            }
        }
        ...
        private static void Add_Slice_Hole()
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                if (i == 0)
                {
                    ...
                    foreach (string outlayer_ in layers.Outer)
                    {
                        Gen.COM($"affected_layer,name={outlayer_},mode=single,affected=no");
                    }
                }
                else if (i == 1)
                {
        ...
        private static void Add_Inn_Hole()
        {
            for (int i = 0; i < 2; i++)
            {
                ...
                else if (i == 1)
                {
                    ...
                    Gen.COM($"affected_layer,name=gbe,mode=single,affected=yes");
                }

                while (limit_y > symbol_inn_y1 || limit_x > symbol_inn_x1)
                {
