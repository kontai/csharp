...
namespace Panel_Script
{
    public static class MyPanelScript
    {
        ...
        private static void Add_Pin_pad(Tooling print_pin)
        {
            //增加外框印刷PIN PAD
            ...
            for (int i = 0; i < print_pin.PrintPinOuterX.Count; i++)
            {
                ...
                if (print_pin.Ccd_Pad_Loc.IndexOf(i) != -1)
                {
                    string ccd_pad = "";
                    //外層正片
                    ccd_pad = mainWindow.outter_pos.IsChecked == true ? sym.Pnl_Pin_Ccd_Pad_P : sym.Pnl_Pin_Ccd_Pad_N;
