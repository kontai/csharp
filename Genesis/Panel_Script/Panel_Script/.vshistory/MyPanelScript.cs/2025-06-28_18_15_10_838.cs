...
namespace Panel_Script
{
    public static class MyPanelScript
    {
        ...
        public static void AutoPanel()
        {
            ...
            double prof_limit_x = DoubleRound(double.Parse(Gen.GetInfo("gPROF_LIMITSxmin")[0]));
            double prof_limit_y = DoubleRound(double.Parse(Gen.GetInfo("gPROF_LIMITSymin")[0]));

            string auto_step = mainWindow.stpSlect.Text;
            string auto_angle = mainWindow.shipRotate.Text;
            if (auto_angle == "0")
            {
                Gen.COM($"sr_tab_add,line=0,step={auto_step},x={-prof_limit_x},y={-prof_limit_y}," +
                        $"nx={Xstep_num},ny={ystep_num},dx={Six + xstep_spec},dy={Siy + ystep_spec}");
            }
            else
            {
                Gen.COM($"sr_tab_add,line=0,step={auto_step},x={Siy+prof_limit_y},y={Six+prof_limit_x}," +
