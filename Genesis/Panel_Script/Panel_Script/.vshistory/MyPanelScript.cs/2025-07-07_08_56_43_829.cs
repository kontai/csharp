...
namespace Panel_Script
{
    public static class MyPanelScript
    {
        ...
        public static void RunPanelScript()
        {
            ...
            Add_Pin_pad(print_pin, OUT_INNER_SMSSTENTVIA.OUTTER);


            Add_Ten_Pad();
            Add_L_Mark();

            //CCD 孔 外層clearance pad
            ...
            Add_Pin_pad(print_pin, OUT_INNER_SMSSTENTVIA.SMSSTENTVIA);
            Add_Pin_pad(print_pin,OUT_INNER_SMSSTENTVIA.INNER);
        ...
        private static void Add_Pin_pad(Tooling print_pin, OUT_INNER_SMSSTENTVIA out_inner_smsstentvia)
        {
            if (out_inner_smsstentvia == OUT_INNER_SMSSTENTVIA.OUTTER)
            {
                //增加外框印刷PIN PAD
                foreach (string outer in layers.Outer)
                {
            ...
            else if (out_inner_smsstentvia == OUT_INNER_SMSSTENTVIA.SMSSTENTVIA)
            {
                ...
                for (int i = 0; i < print_pin.PrintPinOuterX.Count; i++)
                {
                    Gen.COM("add_pad" +
                            ",attributes=no" +
                            $",x={print_pin.PrintPinOuterX[i]},y={print_pin.PrintPinOuterY[i]}" +
                            ",symbol=donut_r180x150" +
                            $",polarity=positive" +
                            $",angle=0,mirror=no" +
                            $",nx=1,ny=1,dx=0,dy=0" +
                            $",xscale=1,yscale=1");
                }
            }else if (out_inner_smsstentvia == OUT_INNER_SMSSTENTVIA.INNER)
            {
