...
namespace Panel_Script
{
    public static class MyPanelScript
    {
        ...
        private static void Add_Mount_Hole()
        {
            ...
            double mnt_pad_y1 = (max_y - min_y) / 2;
            double mnt_pad_y2 = max_y + (Wiy - Frame_x) / 2 - 1.5;
            double mnt_pad_y3 = min_y - (Wiy - Frame_x) / 2 + 1.5;
            double mnt_pad_y4 = (max_y - min_y) / 2 - 0.5;
            double mnt_pad_y5 = min_y - (Wiy - Frame_x) / 2 + 1.5;
            ...
            for (int i = 0; i < mnt_pad_list_x.Count; i++)
            {
                ...
                if (i == 0 || i == 3)
                {
                    ...
                    sym_n = $"{sym.Pnl_Inn_Mnt_Dr2_N}";
                }
                else
                {
                    sym_ = $"{sym.Pnl_Inn_Mnt_Dr2}";
                    sym_n = $"{sym.Pnl_Inn_Mnt_Dr_N}";
                }
                ...
                Gen.COM("add_pad" +
                        $",attributes=no,x={mnt_pad_list_x[i]},y={mnt_pad_list_y[i]}" +
                        $",symbol={sym_n},polarity=negative,angle={mnt_dr_ang_list[i]}" +
                        $",mirror=no,nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
            }
