        ...
        public static void RunPanelScript()
        {
            ...
            if (layers.SignalTMP.Count > 2)
            {
                if (!GenTools.Layer_IsExist(Job, workName, "inn"))
                    GenTools.Layer_Create("inn", GenTools.layerType["drill"]);
                if (GenTools.Layer_IsExist(Job, workName, "pin"))
                    GenTools.Layer_Create("pin", GenTools.layerType["drill"]);
            }

            if (!GenTools.Layer_IsExist(Job, workName, "via"))
                GenTools.Layer_Create("via", GenTools.layerType["drill"]);
            if (!GenTools.Layer_IsExist(Job, workName, "t1"))
                GenTools.Layer_Create("t1", GenTools.layerType["drill"]);
            ...
            {
                GenTools.Add_Copper(layers.Outer, -.1, - .1,100, 100, 0.1, 0.1);
            }
            else
            {
                GenTools.Add_Copper(layers.Outer, -.1, - .1,100, 100, 0.1, 0.1);
