        ...
        private static void Add_Special_Symbols()
        {
            ...
            if (layers.SolderMask.Count > 0)
            {
                foreach (string sm in layers.SolderMask)
                {
                    ...
                    double dam_text_size = GenTools.DoubleRound(double.Parse(min_dam_size) * 0.12);
                ...
                if (GenTools.Layer_IsExist(Job, Step, "t1"))
                {
                    ...
                    double no_mir_text = GenTools.DoubleRound(tent_text_y + tent_text_size - 0.65);
                    Gen.COM("add_text" +
                            $",attributes=no,type=string,x={tent_text_x},y={no_mir_text}" +
                            $",text=C,x_size=0.15,y_size=0.15,w_factor=0.833333333333333" +
                            $",polarity=positive,angle=270,mirror=no,fontname=standard");
                    Gen.COM($"affected_layer,name=t1,mode=single,affected=no");
                }
            }
