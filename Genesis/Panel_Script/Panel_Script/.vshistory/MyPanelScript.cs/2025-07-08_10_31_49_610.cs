    ...
    public static class MyPanelScript
    {
                    ...
                    case 0:
                        if (text_lalyer.Count > 0)
                        {
                            foreach (string outer in layers.Outer)
                            {
                                ...
                                {
                                    string lyr_text = "$$JOB TOP $$DATE-MMDDYY";
                                    Gen.COM("add_text,attributes=no,type=string,x=9.7654093504,y=-5.6917004921,text=$$JOB TOP $$DATE-MMDDYY,x_size=0.2,y_size=0.2,w_factor=2,polarity=positive,angle=0,mirror=no,fontname=standard,ver=1");
                                    Gen.COM("add_text" +
                                            ",attributes=no,type=string" +
                                            $",x={add_job_date_x},y={add_job_date_y}" +
                                            $",text=$$JOB TOP $$DATE-MMDDYY,x_size={x_size},y_size={y_size}" +
                                            $",w_factor={w_factor}" +
                                ...
                                else if (layers.Full_Layer_Side[outer] == "bottom")
                                {
                                    ...
                                    string lyr_text = "$$JOB BOT $$DATE-MMDDYY";
                                    double text_len = 23; //廠料文字長度
                                    text_len = GenTools.DoubleRound(text_len * 0.09);
                                    double add_job_date_y_ = min_y + 1.625 + text_len;
                                    Gen.COM("add_text" +
                                            ",attributes=no,type=string" +
                                            $",x={add_job_date_x},y={add_job_date_y}" +
                    ...
                    case 1:
                        if (text_lalyer.Count > 0)
                        {
                            foreach (string sm in layers.SolderMask)
                            {
                                ...
                                {
                                    string lyr_text = @"$$JOB TOP MASK $$DATE-MMDDYY";
                                    Gen.COM("add_text" +
                                            ",attributes=no,type=string" +
                                            $",x={add_job_date_x},y={add_job_date_y}" +
                                            $",text={lyr_text},x_size={x_size},y_size={y_size}" +
                                ...
                                else if (layers.Full_Layer_Side[sm] == "bottom")
                                {
                                    text_angle = 90;
                                    mirror_text = "yes"; //廠料文字鏡像
                                    string lyr_text = @"$$JOB BOT MASK $$DATE-MMDDYY";
                                    double text_len = 23; //廠料文字長度
                                    text_len = GenTools.DoubleRound(text_len * 0.09);
                                    double add_job_date_y_ = min_y + 2.1 + text_len;
                                    Gen.COM("add_text" +
                                            ",attributes=no,type=string" +
                                            $",x={add_job_date_x},y={add_job_date_y}" +
                                            $",text={lyr_text},x_size={x_size},y_size={y_size}" +
                    ...
                    case 2:
                        if (text_lalyer.Count > 0)
                        {
                            foreach (string ss in layers.SilkScreen)
                            {
                                ...
                                {
                                    string lyr_text = @"$$JOB TOP SILK $$DATE-MMDDYY";
                                    Gen.COM("add_text" +
                                            ",attributes=no,type=string" +
                                            $",x={add_job_date_x},y={add_job_date_y}" +
                                            $",text={lyr_text},x_size={x_size},y_size={y_size}" +
                                ...
                                else if (layers.Full_Layer_Side[ss] == "bottom")
                                {
                                    text_angle = 90;
                                    mirror_text = "yes"; //廠料文字鏡像
                                    string lyr_text = @"$$JOB BOT SILK $$DATE-MMDDYY";
                                    double text_len = 23; //廠料文字長度
                                    text_len = GenTools.DoubleRound(text_len * 0.09);
                                    double add_job_date_y_ = min_y + 2.1 + text_len;
                                    Gen.COM("add_text" +
                            ...
                            if (lamCount == 1)
                            {
                                ...
                                for (int j = 0; j < layers.InnerList.Count; j++)
                                {
                                    ...
                                    else
                                    {
                                        text_angle = 90;
                                        mirror_text = "yes"; //廠料文字鏡像
                                        double text_len = 19; //廠料文字長度
                                        text_len = GenTools.DoubleRound(text_len * 0.09);
                                        add_job_date_y = min_y + 0.925 + text_len;
                                    }
        ...
        private static void Add_CCD_Fid_Pad()
        {
            ...
            if (layers.SolderMask.Count > 0)
            {
                ...
                for (int i = 0; i < ccd_fid_pad.X.Count; i++)
                {
                    Gen.COM("add_pad" +
                            ",attributes=no" +
                            $",x={ccd_fid_pad.X[i]},y={ccd_fid_pad.Y[i]}" +
                            $",symbol={sym.Pnl_Ccd_Sm_Fid}" +
                            $",polarity=positive,angle={sm_ccd_pad_ang[i]}" +
