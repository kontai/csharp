...
namespace Panel_Script
{
    public static class MyPanelScript
    {
        ...
        private static void Add_Hole_Inspection()
        {
             ...
             Gen.COM("affected_layer,name=gbx,mode=single,affected=yes");
             Gen.COM("flatten_layer,source_layer=gbx,target_layer=find_drill_size_");
             Gen.COM("affected_layer,name=gbx,mode=all,affected=no");

            //刪除下料邊的孔,保留板內的孔
             Gen.COM("affected_layer,name=find_drill_size_,mode=single,affected=yes");
            ...
            info,out_file=$EZCAM_DIR/tmp/hole,write_mode=replace,args= -t layer -e $JOB/work/find_drill_size_ -d FEATURES -m script,units=inch
            set drl_size = ` cat $EZCAM_DIR/tmp/hole | grep " r" | grep "#P"  | cut -d " " -f4 | cut -c2- | sort -n | uniq `
            rm $EZCAM_DIR/tmp/hole
            set drl_size_list = ( $drl_size_list $drl_size )

            delete_layer,layer=find_drill_size_
        }
