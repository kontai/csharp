...
namespace Panel_Script
{
    public static class MyPanelScript
    {
        ...
        static MyPanelScript()
        {
            ...
            s_ix = double.Parse(mainWindow.six.Text);
            s_iy = double.Parse(mainWindow.siy.Text);
            w_ix = double.Parse(mainWindow.wix.Text);
            w＿iy = double.Parse(mainWindow.wiy.Text);
            xstep_num = int.Parse(mainWindow.xstep_num.Text);
            ystep_num = int.Parse(mainWindow.ystep_num.Text);
            xstep_spec = double.Parse(mainWindow.xstep_spec.Text);
            ystep_spec = double.Parse(mainWindow.ystep_spec.Text);
        }
        ...
        public static void AutoPanel()
        {
            ...
            if (auto_angle == "0")
            {
                Gen.COM($"sr_tab_change,line=1,step={auto_step},x={Frame_Dist_x},y={Frame_Dist_y}," +
                        $"nx={Xstep_num},ny={ystep_num},dx={Six + xstep_spec},dy={Siy + ystep_spec}");
            }
            else
            {
                Gen.COM($"sr_tab_change,line=1,step={auto_step},x={Frame_Dist_x},y={Frame_Dist_y + Siy}," +
