...
namespace Panel_Script
{
    public static class MyPanelScript
    {
        ...
        public static double Six => GenTools.DoubleRound(s_ix); //排板尺寸X

        public static double Siy => GenTools.DoubleRound(s_iy); //排板尺寸Y

        public static double Wix => GenTools.DoubleRound(w_ix); //下料尺寸X

        public static double Wiy => GenTools.DoubleRound(w_iy); //下料尺寸Y
        ...
        public static double Frame_x => GenTools.DoubleRound(frame_x); //排板尺寸X(包括邊距)
        public static double Frame_y => GenTools.DoubleRound(frame_y); //排板尺寸Y(包括邊距)
        public static double Frame_Dist_x => GenTools.DoubleRound(frame_dist_x); //下料邊X
        public static double Frame_Dist_y => GenTools.DoubleRound(frame_dist_y); //下料邊Y
        ...
        static MyPanelScript()
        {
            ...
            s_ix = GenTools.DoubleRound(double.Parse(mainWindow.six.Text));
            s_iy = GenTools.DoubleRound(double.Parse(mainWindow.siy.Text));
            w_ix = GenTools.DoubleRound(double.Parse(mainWindow.wix.Text));
            w_iy = GenTools.DoubleRound(double.Parse(mainWindow.wiy.Text));
            xstep_num = int.Parse(mainWindow.xstep_num.Text);
            ystep_num = int.Parse(mainWindow.ystep_num.Text);
            xstep_spec = GenTools.DoubleRound(double.Parse(mainWindow.xstep_spec.Text));
            ystep_spec = GenTools.DoubleRound(double.Parse(mainWindow.ystep_spec.Text));
        }
        ...
        public static void RunPanelScript()
        {
            ...
            //印刷PIN
            Tooling print_pin = new Tooling(layers.SignalTMP.Count, frame_x, frame_y, 0, 0);

            Add_Pin_pad(print_pin, OUT_INNER.OUTTER);


            //CCD 光學點
            CCD_Fid_Pad ccd_fid_pad = new CCD_Fid_Pad(frame_x, frame_y);
            foreach (string outer in layers.Outer)
            {
                Gen.COM($"affected_layer,name={outer},mode=single,affected=yes");
            }

            List<double> ten_sym_pad = new List<double>()
            {
                GenTools.DoubleRound(frame_x / 3),
                GenTools.DoubleRound(frame_x / 3 * 2),
                GenTools.DoubleRound(frame_x - 0.2),
                GenTools.DoubleRound(frame_x + 0.2),
            }
            List<double> ten_sym_pad_y = new List<double>()
            {
                ...
                GenTools.DoubleRound(-0.2),
                GenTools.DoubleRound(frame_y + 0.2),
            };
