...
namespace Panel_Script
{
    public static class MyPanelScript
    {
        ...
        public static void RunPanelScript()
        {
            ...
            {
                Gen.INFO($"-t layer -e {Job}/{workName}/inn -d EXISTS -m script,units=inch");
                if (Gen.GetInfo("gEXISTS")[0] == "yes") Gen.COM("delete_layer,layer=inn");
                Gen.INFO($"-t layer -e {Job}/{workName}/pin -d EXISTS -m script,units=inch");
        ...
        public static void AutoPanel()
        {
            ...
            double prof_limit_xmin = DoubleRound(double.Parse(Gen.GetInfo("gPROF_LIMITSxmin")[0]));
            double prof_limit_xmax = DoubleRound(double.Parse(Gen.GetInfo("gPROF_LIMITSxmax")[0]));
            double prof_limit_ymin = DoubleRound(double.Parse(Gen.GetInfo("gPROF_LIMITSymin")[0]));
            double prof_limit_ymax = DoubleRound(double.Parse(Gen.GetInfo("gPROF_LIMITSymax")[0]));

            //absolute limits
            prof_limit_ymax = Math.Abs(prof_limit_ymax);
            prof_limit_xmax = Math.Abs(prof_limit_xmax);
            prof_limit_ymin = Math.Abs(prof_limit_ymin);
            prof_limit_xmin = Math.Abs(prof_limit_xmin);
            ...
            if (auto_angle == "0")
            {
                Gen.COM($"sr_tab_add,line=0,step={auto_step},x={prof_limit_xmax-prof_limit_xmin},y={prof_limit_ymax-prof_limit_ymin}," +
                        $"nx={Xstep_num},ny={ystep_num},dx={Six + xstep_spec},dy={Siy + ystep_spec}");
            }
            else
            {
                Gen.COM($"sr_tab_add,line=0,step={auto_step},x={prof_limit_ymin } ,y= {  prof_limit_xmin}," +
