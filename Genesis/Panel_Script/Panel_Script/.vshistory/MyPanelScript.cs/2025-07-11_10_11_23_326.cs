...
namespace Panel_Script
{
    public static class MyPanelScript
    {
        ...
        private static void Add_Dount_Lyr_Reg()
        {
            ...
            foreach (string inner in layers.Inner)
            {
                int symbol_donut_add = (d_num - 1) * 25;
                int symbol_donut_width=80+symbol_donut_add;
                int symbol_donut_height=60+symbol_donut_add;
                sb.Append(symbol_donut_width);
                sb.Append('x');
                sb.Append(symbol_donut_height);
                symbol_dounut_list.Add(sb.ToString());
                sb.Clear();
            }
        }
