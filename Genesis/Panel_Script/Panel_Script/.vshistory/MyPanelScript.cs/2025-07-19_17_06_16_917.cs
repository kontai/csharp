...
namespace Panel_Script
{
    public static class MyPanelScript
    {
        ...
        private static void Export_Drill_Table_Parameter()
        {
            ...
            //建立臨時檔案
            string exceltmpfile = $"c:/tmp/drill_data/{Job}.exceltmpvarfile";

            using (StreamWriter fw = new StreamWriter())
            {
                fw.WriteLine($"set cu_thkness = {mainWindow.cu_thkness.Text}");
                fw.WriteLine($"set JOB_big = {Job.ToUpper()}");
                fw.WriteLine($"set org_sig = {mainWindow.outter_orig_width}");
                fw.WriteLine($"set org_space = {mainWindow.outter_orig_space}");
                fw.WriteLine($"set org_ring = {mainWindow.outter_orig_ring}");
                fw.WriteLine($"set compensate_sig = {mainWindow.outter_work_width}");
                fw.WriteLine($"set compensate_space = {mainWindow.outter_work_space}");
                fw.WriteLine($"set compensate_ring = {mainWindow.outter_work_ring}");
                fw.WriteLine($"set inn_org_sig = {mainWindow.inner_orig_width}");
                fw.WriteLine($"set inn_org_space = {mainWindow.inner_orig_space}");
                fw.WriteLine($"set inn_org_ring = {mainWindow.inner_orig_ring}");
                fw.WriteLine($"set inn_compensate_sig = {mainWindow.inner_work_width}");
                fw.WriteLine($"set inn_compensate_space = {mainWindow.inner_work_space}");
                fw.WriteLine($"set inn_compensate_ring = {mainWindow.inner_work_ring}");
                ...
                fw.WriteLine($"set outer = ( {layers.Outer.ToString() } )");


                /*
                set cu_thkness = 62.992
                set JOB_big = CAB149M6A
                set org_sig = 2
                set org_space = 3
                set org_ring = 4
                set compensate_sig = 5
                set compensate_space = 6
                set compensate_ring = 7
                set inn_org_sig = 11
                set inn_org_space = 12
                set inn_org_ring = 13
                set inn_compensate_sig = 15
                set inn_compensate_space = 16
                set inn_compensate_ring = 17
                set the_step = work
                set isx = 17.6377987
                set isy = 21.1811034
                set issx = 18.8976378
        ...
        private static void Add_Hole_Inspection()
        {
            ...
            Gen.COM($"flatten_layer,source_layer=gbx,target_layer={drill_size_lyr}");
            Gen.COM("affected_layer,mode=all,affected=no");
        ...
        private static void Add_Target_Hole()
        {
            ...
            for (int i = 0; i < add_tag_x.Count; i++)
            {
                Gen.COM("add_pad" +
                        $",attributes=no,x={add_tag_x[i]},y={add_tag_y[i]}" +
                        $",symbol=r125,polarity=negative,angle=0,mirror=no" +
                        $",nx=1,ny=1,dx=0,dy=0,xscale=1,yscale=1");
            }

            Gen.COM($"affected_layer,name=pin,mode=single,affected=no");
            Gen.COM($"affected_layer,name=gbx,mode=single,affected=no");
        }
            ...
            if (layers.Outer.Count > 0)
            {
                foreach (var outer in layers.Outer)
                {
                    ...
                    if (layers.Full_Layer_Side[outer] == "top")
                    {
                        ...
                        layer_name_text_y = layer_frame_y + 0.0576 - 0.03 - 0.03 + step_y;
                        Gen.COM("add_text" +
                                ",attributes=no,type=string" +
                                $",x={layer_name_text_x},y={layer_name_text_y}" +
                                $",text=T,x_size={text_size_x},x_space={text_space_x},y_size={text_size_y}" +
                    ...
                    else if (layers.Full_Layer_Side[outer] == "bottom")
                    {
                        ...
                        bot_layer_name_text_y = layer_frame_y + 0.0576 - 0.03 + 0.23 + tmp_step_y;
                        Gen.COM("add_text" +
                                ",attributes=no,type=string" +
                                $",x={layer_name_text_x},y={bot_layer_name_text_y}" +
                                $",text=B,x_size={text_size_x},x_space={text_space_x},y_size={text_size_y}" +
                                $",w_factor={text_w_factor},,polarity=positive" +
                                $",angle={text_angle},mirror={text_mirr},fontname=standard,ver=1");
                    }
...
COM add_pad,attributes=no,x=13.801538189,y=10.1156823819,symbol=lay-2
        ...
        private static void Do_Patttern_Fill()
        {
            ...
            {
                isMultiLyr = true;
                Layer_Pattern_Fill(layers.Inner, 0, 0,
                    100, 100,
                    0.1, 0.1,
                    "s160",
                    0, 0);
